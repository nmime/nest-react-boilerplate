...
describe("PostgresSocialAuthStore", () => {
  ...
  it("delegates identity, method, and link-token lookups to repositories scoped by tenant", async () => {
    ...
    expect(
      (
        await store.findIdentity(AuthProvider.Telegram, "42", tenantId)
      )._unsafeUnwrap(),
    ).toMatchObject({ id: "identity-id", providerSubject: "42" });
    expect(repositories.identities.findByProviderSubject).toHaveBeenCalledWith(
    ...
    expect(
      (await store.listIdentities("user-id", tenantId))._unsafeUnwrap(),
    ).toHaveLength(1);
    expect(
      (await store.upsertIdentity(identityEntity))._unsafeUnwrap(),
    ).toMatchObject({ id: "identity-id" });
    expect(
      (
        await store.deleteIdentity("identity-id", "user-id", tenantId)
      )._unsafeUnwrap(),
    ).toBe(true);
    ...
    expect(
      (await store.listMethods("user-id", tenantId))._unsafeUnwrap(),
    ).toHaveLength(1);
    expect(
      (await store.countMethods("user-id", tenantId))._unsafeUnwrap(),
    ).toBe(2);
    ...
    expect(
      (
        await store.consumeLinkToken("hash", ExternalAuthIntent.Link, tenantId)
      )._unsafeUnwrap(),
    ).toMatchObject({ id: "link-token-id" });
    expect(
      (await store.revokeLinkToken("hash", tenantId))._unsafeUnwrap(),
    ).toBe(true);
  });

  it("returns null when repositories report no identity or link token", async () => {
    const { repositories, store } = createStore(crypto);
    repositories.identities.findByProviderSubject.mockReturnValueOnce(
      okAsync(null),
    );
    ...
    expect(
      (
        await store.findIdentity(AuthProvider.Telegram, "unknown", tenantId)
      )._unsafeUnwrap(),
    ).toBeNull();
    expect(
      (
        await store.consumeLinkToken(
          "missing",
  ...
  it("encrypts provider tokens with tenant-scoped AAD before persistence", async () => {
    ...
    expect(
      repositories.providerTokens.persistEncryptedToken,
  ...
  it("maps provider-token listing failures to repository errors", async () => {
    ...
    expect(
      (
        await store.revokeProviderTokens("identity-id", tenantId)
  ...
  it("maps provider-token revoke promise failures with fallback messages", async () => {
    ...
    repositories.providerTokens.revokeToken.mockReturnValueOnce(
      nonErrorRejection as ReturnType<
        typeof repositories.providerTokens.revokeToken
      >,
    );
...
describe("PostgresSocialAuthStore env-derived crypto", () => {
  ...
  it("builds an AES-GCM crypto from a hex key and honors a configured key id", async () => {
    ...
    expect(
      repositories.providerTokens.persistEncryptedToken,
    ).toHaveBeenCalledWith(expect.objectContaining({ keyId: "primary" }));
  });
  ...
  it("builds an AES-GCM crypto from a base64 key and defaults the key id", async () => {
    ...
    expect(
      repositories.providerTokens.persistEncryptedToken,
    ).toHaveBeenCalledWith(expect.objectContaining({ keyId: "env" }));
  });
});
...
describe("InMemorySocialAuthStore", () => {
  it("lists identities and enforces owner and tenant scoping on delete", async () => {
    ...
    expect(
      (await store.listIdentities("owner", tenantId))._unsafeUnwrap(),
    ).toHaveLength(1);
    expect(
      (await store.listIdentities("owner", otherTenantId))._unsafeUnwrap(),
    ).toHaveLength(0);
    (
      store as unknown as {
    ...
    expect(
      (
        await store.findIdentity(AuthProvider.Telegram, "100", tenantId)
      )._unsafeUnwrap(),
    ).toBeNull();
    (
      store as unknown as {
    ...
    expect(
      (
        await store.deleteIdentity(created.id, "not-owner", tenantId)
      )._unsafeUnwrap(),
    ).toBe(false);
    expect(
      (
        await store.deleteIdentity(created.id, "owner", otherTenantId)
      )._unsafeUnwrap(),
    ).toBe(false);
    expect(
      (
        await store.deleteIdentity(created.id, "owner", tenantId)
      )._unsafeUnwrap(),
    ).toBe(true);
  });
  ...
  it("refuses to revoke a link token that is already consumed", async () => {
    ...
    expect(
      (
        await store.revokeLinkToken("consumable-hash", tenantId, now)
      )._unsafeUnwrap(),
    ).toBe(false);
    expect(
      (
        await store.revokeLinkToken("missing-hash", tenantId, now)
      )._unsafeUnwrap(),
    ).toBe(false);
  });
