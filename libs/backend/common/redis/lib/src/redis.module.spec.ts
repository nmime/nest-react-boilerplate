...
function resolveClient(options: RedisConfig, token: symbol): RedisClientLike {
  ...
  const configProvider = providers.find(
    (provider): provider is Provider & ValueProvider =>
      isRecordProvider(provider) &&
      provider.provide === RedisConfigService &&
  ...
  // eslint-disable-next-line @typescript-eslint/no-unsafe-argument -- test wiring: the mocked config useValue is intentionally loosely typed
...
describe("RedisModule.forRoot", () => {
  ...
  it("prefers the transient client, then the shared client, for the transient token", () => {
    ...
    expect(
      resolveClient(
        { transientClient: transient, client: shared },
        RedisTransientInjectToken,
      ),
    ).toBe(transient);
    expect(resolveClient({ client: shared }, RedisTransientInjectToken)).toBe(
      shared,
    );
    expect(resolveClient({}, RedisTransientInjectToken)).toBeInstanceOf(
      InMemoryRedisClient,
    );
  });
  ...
  it("closes each unique client exactly once on application shutdown", async () => {
    ...
    const ShutdownService = providers.find(
      (
        ...
        provider.name === "RedisShutdownService",
    );
    if (!ShutdownService) {
