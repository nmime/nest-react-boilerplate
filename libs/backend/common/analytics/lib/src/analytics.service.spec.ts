...
function recordingPlugin(name = 'test'): RecordingPlugin {
  return {
    name,
    track: vi.fn<(payload: AnalyticsPayload) => Promise<void>>().mockResolvedValue(undefined),
    identify: vi.fn<(payload: AnalyticsIdentifyPayload) => Promise<void>>().mockResolvedValue(undefined),
    page: vi.fn<(payload: AnalyticsPagePayload) => Promise<void>>().mockResolvedValue(undefined),
  };
}
...
describe('AnalyticsService', () => {
  ...
  it('exposes the configured environment', () => {
    const service = new AnalyticsService(new AnalyticsConfigService({ environment: 'staging' }));
  ...
  it('dispatches identify payloads with a generated timestamp', async () => {
    ...
    const service = new AnalyticsService(new AnalyticsConfigService({ enabled: true, plugins: [plugin] }));
  ...
  it('does nothing when analytics are disabled', async () => {
    const plugin = recordingPlugin();
    const service = new AnalyticsService(new AnalyticsConfigService({ enabled: false, plugins: [plugin] }));
  ...
  it('isolates plugin failures and keeps other plugins running', async () => {
    ...
    expect(errorSpy).toHaveBeenCalledWith('Analytics plugin "failing" failed', expect.any(Error));
  });

  it('skips plugins that do not implement the dispatched method', async () => {
    const partial: AnalyticsPlugin = { name: 'partial' };
    const service = new AnalyticsService(new AnalyticsConfigService({ enabled: true, plugins: [partial] }));
  ...
  it('guards track against a rejected dispatch to avoid unhandled rejections', async () => {
    ...
    expect(errorSpy).toHaveBeenCalledWith('Analytics track for "order_created" failed', expect.any(Error));
  });
});
