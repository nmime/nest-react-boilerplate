...
describe("auth user stores", () => {
  ...
  it("writes the access-policy cache through the Postgres store", async () => {
    const repository = {
      setAccessPolicy: vi.fn(
        (id: string, policy: { roles?: string[]; permissions?: string[] }) =>
    ...
    expect(
      (await store.setAccessPolicy("missing", { roles: [] }))._unsafeUnwrap(),
    ).toBeNull();
  });
  ...
  it("updates the in-memory access-policy cache and guards tenant/id mismatches", async () => {
    ...
    expect(
      (await store.setAccessPolicy(created.id, {}))._unsafeUnwrap(),
    ).toMatchObject({
      id: created.id,
      roles: ["user", "admin"],
      permissions: ["profile:read", "admin:manage:all"],
    ...
    expect(
      (
        await store.setAccessPolicy(created.id, { roles: [] }, "other-tenant")
      )._unsafeUnwrap(),
    ).toBeNull();
  });
