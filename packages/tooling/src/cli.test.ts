...
describe('repository tooling command surface', () => {
  ...
  it('exposes the locale catalog generator and its staleness check', async () => {
    assert.equal(await main(['i18n', 'catalogs', '--check']), 0);
  });

  it('reports an unknown command instead of exiting zero', async () => {
  ...
  it('dispatches every command a generated locale artifact tells the reader to run', async () => {
    const headers = [
