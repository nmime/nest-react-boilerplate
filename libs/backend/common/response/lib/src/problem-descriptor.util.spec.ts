...
describe('formatProblemDescriptor', () => {
  ...
  it('keeps the code segment while dropping an absent instance', () => {
    expect(formatProblemDescriptor(problem({ code: 'bad-request', status: 400, title: 'Bad Request' }))).toBe(
      '400 Bad Request code=bad-request',
    );
  });

  it('emits only the status and title when code and instance are absent', () => {
    expect(formatProblemDescriptor(problem({}))).toBe('500 Internal Server Error');
  });
});
