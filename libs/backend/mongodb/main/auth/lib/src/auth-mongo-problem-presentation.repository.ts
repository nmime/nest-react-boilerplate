  ...
  private async persist(
    ...
    if (input.expectedRevision === 0) {
      record = {
        ...
        messageRu: input.messageRu?.trim() ?? input.textsRu?.[0]?.trim() ?? '',
        messageZh: input.messageZh?.trim() ?? input.textsZh?.[0]?.trim() ?? '',
        textsEn: [...(input.textsEn ?? (input.messageEn ? [input.messageEn.trim()] : []))],
        textsRu: [...(input.textsRu ?? (input.messageRu ? [input.messageRu.trim()] : []))],
        textsZh: [...(input.textsZh ?? (input.messageZh ? [input.messageZh.trim()] : []))],
        support: input.support ?? false,
        customDescription: input.customDescription?.trim() ?? '',
        figmaOnly: input.figmaOnly ?? false,
        revision: 1,
...
const snapshot = (item: ProblemPresentationRecord): Record<string, unknown> => ({
  ...
  textsEn: item.textsEn ?? (item.messageEn ? [item.messageEn] : []),
  textsRu: item.textsRu ?? (item.messageRu ? [item.messageRu] : []),
  textsZh: item.textsZh ?? (item.messageZh ? [item.messageZh] : []),
  support: item.support ?? false,
  customDescription: item.customDescription ?? '',
  figmaOnly: item.figmaOnly ?? false,
