...
- `pnpm run deploy:validate` is the no-deploy validation bundle for production Docker Compose, Helm, and deployment docs/config hardening. Local runs require Helm because the script renders/checks Helm; CI installs Helm before this gate.
