...
export class Migration20260605143000CreateAdminAuditLogs extends Migration {
  override up(): void {
    ...
    this.addSql(
      'create index if not exists "ix__admin_audit_logs__tenant_created_at" on "admin_audit_logs" ("tenant_id", "created_at");',
    );
    this.addSql(
      'create index if not exists "ix__admin_audit_logs__tenant_action" on "admin_audit_logs" ("tenant_id", "action");',
    );
    this.addSql(
      'create index if not exists "ix__admin_audit_logs__tenant_target_user_id" on "admin_audit_logs" ("tenant_id", "target_user_id");',
    );
  }

  override down(): void {
    this.addSql(
      'drop index if exists "ix__admin_audit_logs__tenant_target_user_id";',
    );
    this.addSql('drop index if exists "ix__admin_audit_logs__tenant_action";');
    this.addSql(
      'drop index if exists "ix__admin_audit_logs__tenant_created_at";',
    );
