...
export function hashPassword(password: string, salt = randomBytes(16).toString('base64url')): string {
  const digest = pbkdf2Sync(password, salt, PasswordIterations, PasswordKeyLength, 'sha256').toString('base64url');
...
export function verifyPassword(password: string, encodedHash: string): boolean {
  ...
  const digest = pbkdf2Sync(password, salt, parsedIterations, expected.length, 'sha256');
...
function parsePasswordHashIterations(value: string | undefined): number | null {
  ...
  if (!Number.isInteger(parsed) || String(parsed) !== value || parsed < 1 || parsed > MaximumPasswordHashIterations) {
    return null;
  }
