...
const optionalBoolean = Joi.boolean().truthy('1', 'true', 'yes', 'on').falsy('0', 'false', 'no', 'off').optional();
...
export const natsEnvSchema = Joi.object<NatsEnvironment>({
  ...
  NATS_DRAIN_TIMEOUT_MS: Joi.number().integer().positive().empty('').default(5000),
});
