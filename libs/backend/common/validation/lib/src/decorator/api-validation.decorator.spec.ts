...
describe('ApiValidation', () => {
  ...
  it('registers a configured ValidationPipe on the decorated class', () => {
    ...
    const pipes = Reflect.getMetadata(PIPES_METADATA, DecoratedController) as unknown[];
  ...
  it('registers a configured ValidationPipe on a decorated method', () => {
    ...
    const handler: unknown = Object.getOwnPropertyDescriptor(DecoratedController.prototype, 'handle')?.value;
