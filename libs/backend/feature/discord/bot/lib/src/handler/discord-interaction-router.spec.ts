...
import { DiscordAccountService } from '../service/discord-account.service';
import { DiscordCustomIdCodec, type DiscordCustomIdAction } from '../service/discord-custom-id.codec';
...
import { componentValidationMessage } from '../util';
...
function command(name: string, subcommand?: string, overrides: Record<string, unknown> = {}) {
...
function validComponentId(
  ...
  options: { expiresAt?: Date; locale?: string } = {},
) {
...
describe('DiscordInteractionRouter', () => {
  ...
  it('routes /account link through auth link builder and localizes by Discord locale', async () => {
    ...
    expect(buttons(response).map((button) => button.label)).toEqual(['Привязать аккаунт', 'Отмена']);
    expect(buttons(response)[0]?.url).toBe('https://example.test/auth/discord?state=opaque');
  ...
  it('returns actionable missing-config text instead of a broken link URL', async () => {
    ...
    const response = await setup.router.route(command('account', 'link'), context({ webAppBaseUrl: undefined }));

    expectEphemeral(response);
    expect(text(response)).toContain('not configured yet');
    expect(text(response)).toContain('Discord auth');
    expect(buttons(response).map((button) => button.label)).toEqual(['Back', 'Home']);
  ...
  it('returns localized link error when auth is unavailable', async () => {
    ...
    const response = await setup.router.route(command('account', 'link', { locale: 'ru' }), context());
  ...
  it('routes /account status for linked and unlinked accounts with localized components', async () => {
    ...
    const linked = await harness(linkedAccount).router.route(command('account', 'status'), context());
    ...
    expect(buttons(linked).map((button) => button.label)).toEqual(['Unlink account', 'Home']);
    ...
    expect(buttons(unlinked).map((button) => button.label)).toEqual(['Привязать аккаунт', 'Домой']);
    expect(buttons(unlinked)[0]?.style).toBe(ButtonStyle.Primary);
  });

  it('returns an actionable error when status lookup fails', async () => {
    const account = new DiscordAccountService({
      createDiscordAuthorizationRequest: vi.fn(),
      listProviderIdentities: vi.fn().mockRejectedValue(new Error('db down')),
    });
    const response = await harness(account).router.route(command('account', 'status'), context());

    expectEphemeral(response);
    expect(text(response)).toBe('Bot service is temporarily unavailable.');
  });

  it('unlinks the account when the confirm action succeeds', async () => {
    ...
    const setup = harness(account);
    const response = await setup.router.route(component(validComponentId(setup, 'confirm')), context());

    expectUpdate(response);
    expect(text(response)).toBe('Discord account unlinked.');
    expect(buttons(response)).toEqual([]);
    expect(unlinkProviderIdentity).toHaveBeenCalledWith('identity-1', expect.objectContaining({ subject: user.id }));
  });

  it('keeps the unlink failure response when the account cannot be unlinked', async () => {
    const account = new DiscordAccountService({
      createDiscordAuthorizationRequest: vi.fn(),
      listProviderIdentities: vi.fn().mockResolvedValue([]),
      unlinkProviderIdentity: vi.fn().mockResolvedValue({ unlinked: false }),
    });
    const setup = harness(account);
    const response = await setup.router.route(component(validComponentId(setup, 'confirm')), context());

    expectUpdate(response);
    expect(text(response)).toBe('Could not unlink your account from the bot.');
    expect(buttons(response).map((button) => button.label)).toEqual(['Back', 'Home']);
  });
  ...
  it('routes /help with configured and missing app URLs without broken buttons', async () => {
    ...
    const missingApp = await setup.router.route(command('help'), context({ webAppBaseUrl: undefined }));

    expectEphemeral(help);
    expect(text(help)).toBe('Use /account link to connect your Discord account and /account status to check it.');
    expect(buttons(help).map((button) => button.label)).toEqual(['Link account', 'Check status', 'Home', 'Open App']);
  ...
  it('falls back for unknown command, subcommand, modal, and unsupported type', async () => {
    const setup = harness();
    const unknownCommand = await setup.router.route(command('unknown'), context());
    const unknownSubcommand = await setup.router.route(command('account', 'unlink'), context());
    ...
    expect(text(unknownCommand)).toBe('The bot could not complete this action.');
    expect(text(unknownSubcommand)).toBe('The bot could not complete this action.');
    expect(text(modal)).toBe('Бот не смог выполнить это действие.');
    expect(text(unsupported)).toBe('Бот не смог выполнить это действие.');
  });

  it('rejects component interactions whose payload has no custom id', async () => {
    const setup = harness();
    const response = await setup.router.route(
      component('', { data: { component_type: ComponentType.Button } }),
      context(),
    );

    expectEphemeral(response);
    expect(text(response)).toBe('This bot action could not be verified. Please start again.');
  });

  it('maps unexpected component validation errors to an expired-action message', () => {
    expect(componentValidationMessage(new Error('boom'), 'en')).toBe('This bot action expired. Please start again.');
  });

  it.each([
    [
      'tampered',
      (setup: ReturnType<typeof harness>) => validComponentId(setup, 'home').replace(':h:', ':l:'),
    ...
    [
      'wrong owner',
      (setup: ReturnType<typeof harness>) => validComponentId(setup, 'home', { userId: '999999999999999999' }),
      'This bot action belongs to another Discord user.',
    ],
    [
      'wrong guild',
      (setup: ReturnType<typeof harness>) => validComponentId(setup, 'home', { guildId: '345678901234567890' }),
      'This bot action belongs to another Discord server.',
    ],
    [
      'wrong tenant',
      (setup: ReturnType<typeof harness>) => validComponentId(setup, 'home', { tenantId: '1' }),
      'This bot action belongs to another workspace.',
    ],
    [
      'missing nonce',
      (setup: ReturnType<typeof harness>) =>
        setup.customIds.encode({ action: 'home', userId: user.id, guildId, tenantId }, { secret }),
      'This bot action expired. Please start again.',
    ],
  ])('returns localized ephemeral component error for %s custom_id', async (_name, id, expected) => {
    const setup = harness();
    const response = await setup.router.route(component(id(setup)), context());
  ...
  it.each([
    ['home', 'Use /account link to connect your Discord account and /account status to check it.'],
    ['back', 'Your Discord account is not linked.'],
    ['link', 'Use the button below to link your Discord account.'],
    ['unlink', 'Confirm that you want to unlink your Discord account from this workspace.'],
    ['confirm', 'Could not unlink your account from the bot.'],
  ] as const)('updates the existing message for %s component action', async (action, expected) => {
    const setup = harness();
    const response = await setup.router.route(component(validComponentId(setup, action)), context());
  ...
  it('falls back to the interaction locale when navigation state locale is blank', async () => {
    const setup = harness();
    const response = await setup.router.route(
      component(validComponentId(setup, 'home', {}, { locale: '' }), {
        locale: 'ru',
      }),
      context(),
    );
    ...
    expectUpdate(response);
    expect(text(response)).toContain('Используйте /account link');
  });
  ...
  it('does not render raw template delimiters in Discord responses', async () => {
    ...
    const responses = await Promise.all([
      ...
      setup.router.route(component(validComponentId(setup, 'open_app')), context({ webAppBaseUrl: undefined })),
    ]);

    for (const response of responses) {
      const rendered = [text(response), ...buttons(response).flatMap((button) => [button.label, button.url])].join(
        '\n',
      );
  ...
  it('opens the configured web app URL for the open_app component action', async () => {
    const setup = harness();
    const response = await setup.router.route(component(validComponentId(setup, 'open_app')), context());

    expectEphemeral(response);
    expect(text(response)).toBe('https://app.example.test');
  });

  it('rejects an interaction that carries no Discord user', async () => {
    await expect(harness().router.route(command('help', undefined, { user: undefined }), context())).rejects.toThrow(
      'Discord interaction user missing.',
    );
  });
});
