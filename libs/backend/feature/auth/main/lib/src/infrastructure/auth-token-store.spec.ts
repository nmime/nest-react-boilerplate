...
describe("InMemoryAuthTokenStore", () => {
  it("rotates and revokes refresh tokens per tenant", async () => {
    ...
    await expect(
      store.revokeRefreshToken("missing-token", tenantA),
    ).resolves.toMatchObject({
      value: false,
    });
    await expect(
  ...
  it("consumes email verification and password reset tokens once", async () => {
    ...
    await expect(
      store.consumeUserActionToken("missing-token", "password_reset", tenantA),
    ).resolves.toMatchObject({ value: null });
...
describe("PostgresAuthTokenStore", () => {
  ...
  it("rotates, revokes, finds, and consumes tokens by hash and maps entities back", async () => {
    ...
    expect(
      (await store.revokeRefreshToken("old-token", tenantA))._unsafeUnwrap(),
    ).toBe(true);
    expect(repository.revokeRefreshToken).toHaveBeenCalledWith(
      hashOpaqueToken("old-token"),
      tenantA,
    );

    expect(
      (await store.findRefreshToken("old-token", tenantA))._unsafeUnwrap(),
    ).toMatchObject({ id: "rotated-id", tokenHash: "rotated-hash" });
    expect(
      (
        await store.consumeUserActionToken(
          "action-token",
          "password_reset",
          tenantA,
        )
  ...
  it("returns null when rotation, lookup, or consumption find nothing", async () => {
    ...
    expect(
      (
        await store.consumeUserActionToken("t", "email_verification")
      )._unsafeUnwrap(),
    ).toBeNull();
  });
  ...
  it("maps repository failures to token store errors with a stable fallback message", async () => {
    ...
    expect(
      (await withMessage.rotateRefreshToken("t"))._unsafeUnwrapErr(),
    ).toEqual({ code: "token_store_error", message: "db offline" });
    expect(
      (await withoutMessage.revokeRefreshToken("t"))._unsafeUnwrapErr(),
    ).toEqual({
