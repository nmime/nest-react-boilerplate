...
- Read `../../../docs/dependency-management.md`, `../../../docs/supply-chain.md`,
  root package-manager policy, `pnpm-workspace.yaml`, the owning package
  manifest, lockfile rules, and release notes or migration guide for the exact version range.
...
3. Use pnpm to update the lockfile; never hand-edit `pnpm-lock.yaml`. Preserve pinned Node 24 and pnpm 11.11 constraints.
...
Run focused lint, typecheck, tests, and builds plus broader consumers for shared packages. Run the repository dependency audit/check commands and `git diff --check`. Separate registry/network blockers from compatibility failures.
