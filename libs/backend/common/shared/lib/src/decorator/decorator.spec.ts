...
import { MaxObjectSize, MaxObjectSizeMetadataKey } from './max-object-size.decorator';
...
function paramFactoryOf(enhancer: () => ParameterDecorator): ParamFactory {
  ...
  const metadata = Reflect.getMetadata(ROUTE_ARGS_METADATA, Target, 'run') as Record<string, { factory: ParamFactory }>;
...
describe('Geo param decorator', () => {
  it('reads the resolved geo data off the request', () => {
    ...
    expect(factory(undefined, httpContext({ geo: { country: 'US' } }))).toEqual({ country: 'US' });
  });
});
...
describe('Ip param decorator', () => {
  it('reads the request ip', () => {
    ...
    expect(factory(undefined, httpContext({ ip: '203.0.113.7' }))).toBe('203.0.113.7');
  });
});
...
describe('Cron / MaxObjectSize metadata decorators', () => {
  ...
  it('attaches the cron expression as method metadata', () => {
    expect(Reflect.getMetadata(CronExpressionMetadataKey, Jobs.prototype.sync)).toBe('*/5 * * * *');
  });

  it('attaches the max object size as method metadata', () => {
    expect(Reflect.getMetadata(MaxObjectSizeMetadataKey, Jobs.prototype.upload)).toBe(1024);
  });
});
