...
function apiMock(
  ...
  const fetchMock = vi.fn(
    async (input: RequestInfo | URL, init?: RequestInit) => {
      ...
      if (
        options.failSendMessageText &&
        method === "sendMessage" &&
        payload.text === options.failSendMessageText
      ) {
        return new Response(
          JSON.stringify({
            ok: false,
...
function latestPayload(
  ...
  return (
    [...calls].reverse().find((call) => call.method === method)?.payload ?? {}
  );
}
...
function buttonByText(
  ...
  const button = flattenButtons(payload).find((candidate) =>
...
describe("createTelegramBot", () => {
  ...
  it("falls back to localized link text when instructions are unavailable from a menu callback", async () => {
    ...
    await bot.handleUpdate(
      callbackUpdate(
        callbackDataFor(latestPayload(calls, "sendMessage"), "Link"),
      ) as never,
    );
    await bot.handleUpdate(
      callbackUpdate(
        callbackDataFor(
          latestPayload(calls, "editMessageText"),
  ...
  it("edits existing messages for callback navigation and keeps Back and Home compact", async () => {
    ...
    await bot.handleUpdate(messageUpdate("/start") as never);
    const secondSettings = callbackDataFor(
      latestPayload(calls, "sendMessage"),
      "Settings",
    );
    await bot.handleUpdate(callbackUpdate(secondSettings) as never);
    const home = callbackDataFor(
      latestPayload(calls, "editMessageText"),
      "Home",
    );
  ...
  it("traverses profile, link, language, support, and home menu callbacks", async () => {
    const { calls, fetchMock } = apiMock();
    const createLinkInstructions = vi.fn(() =>
      Promise.resolve("Open this Telegram-only link from your account page."),
    );
    const updateLinkedUserLocale = vi.fn(() => Promise.resolve(undefined));
    const auth: TelegramBotAuthPort = {
      consumeLinkPayload: vi.fn(() => Promise.resolve(null)),
    ...
    await bot.handleUpdate(
      callbackUpdate(
        callbackDataFor(latestPayload(calls, "sendMessage"), "Profile"),
      ) as never,
    );

    expect(latestPayload(calls, "editMessageText").text).toContain(
      "Telegram auth.social.status.linked",
    );

    await bot.handleUpdate(
      callbackUpdate(
        callbackDataFor(latestPayload(calls, "editMessageText"), "Link"),
      ) as never,
    );
    expect(latestPayload(calls, "editMessageText").text).toBe(
      "Starting account link.",
    ...
    expect(createLinkInstructions).toHaveBeenCalledWith(
      expect.objectContaining({ providerSubject: "100" }),
    );
    expect(latestPayload(calls, "editMessageText").text).toBe(
      "Open this Telegram-only link from your account page.",
    );

    await bot.handleUpdate(
      callbackUpdate(
        callbackDataFor(latestPayload(calls, "editMessageText"), "Language"),
      ) as never,
    );
    expect(latestPayload(calls, "editMessageText").text).toBe(
    ...
    await bot.handleUpdate(
      callbackUpdate(
        callbackDataFor(latestPayload(calls, "editMessageText"), "Russian"),
      ) as never,
    );
    expect(updateLinkedUserLocale).toHaveBeenCalledWith(
      expect.objectContaining({
    ...
    await bot.handleUpdate(
      callbackUpdate(
        callbackDataFor(latestPayload(calls, "editMessageText"), "Назад"),
      ) as never,
    );
    expect(latestPayload(calls, "editMessageText").text).toBe(
      "Выберите язык бота.",
    ...
    await bot.handleUpdate(
      callbackUpdate(
        callbackDataFor(latestPayload(calls, "sendMessage"), "Support"),
      ) as never,
    );
    expect(latestPayload(calls, "editMessageText").text).toBe(
  ...
  it("warns users when callback message edits fail", async () => {
    ...
    const support = callbackDataFor(
      latestPayload(failingApi.calls, "sendMessage"),
      "Support",
    );
  ...
  it("covers secondary menu navigation buttons", async () => {
    const { calls, fetchMock } = apiMock();
    const { bot } = createTelegramBot(
      config({ rateLimit: { timeFrameMs: 10, limit: 100 } }),
      { fetch: fetchMock },
    );
    ...
    await bot.handleUpdate(
      callbackUpdate(
        callbackDataFor(latestPayload(calls, "editMessageText"), "Back"),
      ) as never,
    );
    expect(latestPayload(calls, "editMessageText").text).toBe(
      "Welcome! Choose an action.",
    ...
    await bot.handleUpdate(
      callbackUpdate(
        callbackDataFor(latestPayload(calls, "editMessageText"), "Settings"),
      ) as never,
    );
    expect(latestPayload(calls, "editMessageText").text).toBe(
      "Opening settings.",
    ...
    await bot.handleUpdate(
      callbackUpdate(
        callbackDataFor(latestPayload(calls, "editMessageText"), "Home"),
      ) as never,
    );
    expect(latestPayload(calls, "editMessageText").text).toBe(
      "Welcome! Choose an action.",
    ...
    await bot.handleUpdate(
      callbackUpdate(
        callbackDataFor(latestPayload(calls, "sendMessage"), "Settings"),
      ) as never,
    );
    await bot.handleUpdate(
      callbackUpdate(
    ...
    await bot.handleUpdate(
      callbackUpdate(
        callbackDataFor(latestPayload(calls, "editMessageText"), "English"),
      ) as never,
    );
    expect(latestPayload(calls, "editMessageText").text).toBe(
      "Choose your bot language.",
    ...
    await bot.handleUpdate(
      callbackUpdate(
        callbackDataFor(latestPayload(calls, "editMessageText"), "Support"),
      ) as never,
    );
    expect(latestPayload(calls, "editMessageText").text).toBe(
      "Opening support options.",
  ...
  it("logs when a rate-limit notice cannot be sent", async () => {
    ...
    ).toContain("Telegram bot rate-limit reply failed");
    stderrWrite.mockRestore();
  });
});
