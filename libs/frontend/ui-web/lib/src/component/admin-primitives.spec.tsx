...
import { AdminSearchFilterToolbar, UiSearchFilterToolbar } from './search-filter-toolbar';
...
const rows: UserRow[] = [{ email: 'ada@example.com', id: 'usr_1', status: 'active' }];
...
describe('admin UI primitives', () => {
  ...
  it('supports AdminDataTable alias export and table error action callbacks', () => {
    ...
    expect(screen.getByRole('alert').textContent).toContain('The API timed out');
  ...
  it('renders standalone UiTable primitives with semantic classes', () => {
    ...
    expect(screen.getByRole('table', { name: 'Standalone table' })).toBeTruthy();
    expect(screen.getByRole('columnheader', { name: 'Header' })).toBeTruthy();
    expect(screen.getByRole('cell', { name: 'Cell value' })).toBeTruthy();
    expect(screen.getByRole('row', { name: 'Cell value' }).className).toContain('xr-table__row');
  });

  it('renders table loading, empty, and resource-error states', () => {
    const { rerender } = render(<UiDataTable columns={columns} isLoading rowKey={(row) => row.id} rows={[]} />);

    expect(screen.getByRole('status').textContent).toContain('Loading records');

    rerender(<UiDataTable columns={columns} rowKey={(row) => row.id} rows={[]} />);
    expect(screen.getByRole('status').textContent).toContain('No records found');

    rerender(<UiDataTable columns={columns} error="The API timed out" rowKey={(row) => row.id} rows={[]} />);
    expect(screen.getByRole('alert').textContent).toContain('The API timed out');
  });
  ...
  it('renders pagination with native buttons and page semantics', () => {
    ...
    render(<UiPagination currentPage={2} onPageChange={onPageChange} pageSize={10} totalItems={35} totalPages={4} />);
  ...
  it('disables pagination edge buttons and keeps page callbacks bounded', () => {
    const onPageChange = vi.fn();
    const { rerender } = render(<UiPagination currentPage={1} onPageChange={onPageChange} totalPages={3} />);
    ...
    rerender(<UiPagination currentPage={3} onPageChange={onPageChange} pageSize={25} totalItems={60} totalPages={3} />);
  ...
  it('renders search/filter toolbar without unlabelled or native ugly controls', () => {
    ...
    expect(screen.getByRole('search', { name: 'Search and filters' })).toBeTruthy();
  ...
  it('keeps toolbar filters outside the search form so Radix selects stay semantic-only', () => {
    ...
    expect(screen.getByRole('combobox', { name: 'Filter users by status' })).toBeTruthy();
    expect(document.querySelectorAll('select')).toHaveLength(0);
    ...
    const labelCountFor = (control: HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement): number =>
  ...
  it('supports AdminSearchFilterToolbar alias, submit, custom labels, and partial modes', () => {
    ...
    expect(screen.getByRole('searchbox', { name: 'Find user' }).getAttribute('placeholder')).toBe('Email or ID');
    fireEvent.submit(screen.getByRole('search', { name: 'User tools' }));
    expect(onSubmit).toHaveBeenCalledWith('ada');

    rerender(<AdminSearchFilterToolbar actions={<UiButton>Create</UiButton>} label="Actions only" />);
  ...
  it('opens dropdown menus with item roles, disabled state, selection, and focus', () => {
    ...
    expect(screen.getByRole('menuitem', { name: 'Suspend' }).getAttribute('aria-disabled')).toBe('true');
    expect(screen.getByRole('menuitem', { name: 'Delete' }).className).toContain('xr-menu__item--warning');
    archive.focus();
  ...
  it('supports confirm dialog confirm, cancel, and open-change callbacks', () => {
    ...
    expect(onOpenChange).toHaveBeenCalledWith(true);
    expect(screen.getByRole('alertdialog', { name: 'Delete user' })).toBeTruthy();
  ...
  it('supports tabs arrow activation, value callbacks, and disabled tabs', () => {
    ...
    expect(screen.getByRole<HTMLButtonElement>('tab', { name: 'Danger' }).disabled).toBe(true);
    profile.focus();
  ...
  it('handles checkbox and switch changes, disabled state, and descriptions', () => {
    ...
    render(
      <>
        <UiCheckbox description="Selects the current user row" label="Select row" onCheckedChange={onCheckboxChange} />
        <UiSwitch description="Send lifecycle emails" label="Notifications" onCheckedChange={onSwitchChange} />
  ...
  it('renders notification, copyable text, resource errors, and status tags', () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: 'Copy value: usr_123' }));
    expect(writeText).toHaveBeenCalledWith('usr_123');
    expect(screen.getByRole('alert').textContent).toContain('Resource unavailable');
    expect(screen.getByText('Provisioning').getAttribute('data-tone')).toBe('warning');
  });
  ...
  it('supports notification warning/action variants, resource error props, and status tones', () => {
    ...
    expect(screen.getByRole('alert', { name: 'Users failed' }).textContent).toContain('Custom failure');
