...
function buildTenantReplacements(previous: NrbConfig['tenant'], next: NrbConfig['tenant']): IdentityReplacement[] {
  ...
  // Match seed users by email (their stable identity) instead of by array index,
  // so a removed trailing user does not shift the surviving users' rewrites.
  const nextByEmail = new Map(next.seed.users.map((user) => [user.email.toLowerCase(), user]));
  for (const p of previous.seed.users) {
    const n = nextByEmail.get(p.email.toLowerCase());
    if (n) {
      add(p.email, n.email, 'tenant:seed:user:email');
      add(p.name, n.name, 'tenant:seed:user:name');
      add(p.password, n.password, 'tenant:seed:user:password');
    } else {
      // Removal — strip the removed user's literals from engine-managed files.
      // Structured seed files are additionally regenerated by setup.
      add(p.email, '', 'tenant:seed:removed:email');
      add(p.name, '', 'tenant:seed:removed:name');
      add(p.password, '', 'tenant:seed:removed:password');
    }
  }
  return out;
}
...
/**
 * Literals whose replacement target is the empty string (removed seed users).
 * Their values are deliberately erased from managed file content, so they
 * must never survive in persisted reconfigure metadata either — the engine
 * uses this list to scrub `originalContent` snapshots before they are
 * recorded in `.nrb/identity.json`.
 */
export function removalLiterals(previous: NrbConfig, next: NrbConfig): string[] {
  return buildOrderedReplacements(previous, next)
