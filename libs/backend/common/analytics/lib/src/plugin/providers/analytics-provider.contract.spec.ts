...
describe('analytics providers', () => {
  ...
  it('exports provider classes from provider folders', () => {
    expect(Ga4MeasurementProtocolProvider.prototype).toBeInstanceOf(AbstractAnalyticsProvider);
    expect(LoggerAnalyticsProvider.prototype).toBeInstanceOf(AbstractAnalyticsProvider);
    expect(NoopAnalyticsProvider.prototype).toBeInstanceOf(AbstractAnalyticsProvider);
    expect(PostHogAnalyticsProvider.prototype).toBeInstanceOf(AbstractAnalyticsProvider);
    expect(UmamiAnalyticsProvider.prototype).toBeInstanceOf(AbstractAnalyticsProvider);
  });
});
