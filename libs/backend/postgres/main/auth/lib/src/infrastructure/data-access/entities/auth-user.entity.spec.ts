...
describe('AuthUserEntity', () => {
  ...
  it('registers table, primary key, partial unique tenant email, and access metadata', () => {
    ...
    expect(metadata.properties.passwordHash.fieldNames).toContain('password_hash');
    ...
    expect(metadata.uniques).not.toContainEqual(expect.objectContaining({ name: 'uq__auth_users__tenant_id_email' }));
  });
