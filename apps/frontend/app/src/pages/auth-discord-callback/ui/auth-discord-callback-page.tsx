...
import { UiAlert, UiCard, UiLoading, UiSection, UiStatCard, UiStatusPill, UiToast } from '../../../shared/ui';
...
export function AuthDiscordCallbackPage({ navigate }: Readonly<AuthDiscordCallbackPageProps>) {
  ...
  return (
    ...
    >
      <div className="xr-callback-layout" data-design-marker="discord-callback-v3">
        <UiCard className="xr-callback-brief xr-surface-glow" title="OAuth return desk">
          <div className="xr-card-stack">
            <UiAlert className="xr-inline-alert" tone={hasRequiredQuery ? 'info' : 'warning'}>
            ...
            <div className="xr-stat-grid xr-stat-grid--compact">
              ...
              />
              <UiStatCard detail="session exchange" label="Callback" value={socialAuth.discordCallbackStatus} />
            </div>
          </div>
        </UiCard>
        <UiCard className="xr-callback-card xr-surface-glow" title={t('auth.social.discord.callback.title')}>
          <div className="xr-status-row">
            <span className="xr-status-heading">{t('auth.provider.discord')}</span>
            <UiStatusPill
              label={hasRequiredQuery ? socialAuth.discordCallbackStatus : 'missing'}
              live={socialAuth.isDiscordCallbackPending ? 'polite' : 'off'}
              tone={getDiscordCallbackTone(socialAuth.discordCallbackStatus, hasRequiredQuery)}
            />
          </div>
          ...
          {!hasRequiredQuery ? (
            <UiToast message={t('auth.social.discord.callback.missingState')} tone="warning" />
          ) : null}
