...
describe("AuthService", () => {
  ...
  it("bootstraps by ROLE and refreshes the jsonb cache identically to the shared matrix", async () => {
    ...
    const normalPolicy = createDefaultAccessPolicy(
      "member@example.com",
      process.env,
    );
    ...
    const assignedRoleKeys = (
      await roles.listRoleKeys(adminSession.user.id)
    )._unsafeUnwrap();
    expect(
      [...assignedRoleKeys].sort((left, right) => left.localeCompare(right)),
    ).toEqual(["admin", "user"]);

    // The persisted cache (what createAuthSession reads on the hot path) matches.
    const persistedAdmin = (
      await users.findById(adminSession.user.id)
    )._unsafeUnwrap();
    ...
    expect(
      validateBearerAuthorization(
        bearerAuthorization(adminSession.accessToken),
        {
          AUTH_JWT_SECRET: testJwtSecretValue,
        },
      ),
  ...
  it("does not rotate the refresh token when the user is inactive", async () => {
    ...
    await expect(
      service.refreshSession({ refreshToken: "token" }),
  ...
  it("handles refresh-token, revocation, and user-action token fallback branches", async () => {
    ...
    const missingRefreshService = new AuthService(
      missingUsers as AuthUserStore,
      missingTokens as AuthTokenStore,
    );

    await expect(
      missingRefreshService.refreshSession({ refreshToken: "missing-token" }),
    ).rejects.toThrow("Invalid refresh token");
    expect(rotateMissing).toHaveBeenCalledWith(
      "missing-token",
      DefaultAuthTenantId,
    );
    ...
    const unrotatableService = new AuthService(
      activeUsers as AuthUserStore,
      unrotatableTokens as AuthTokenStore,
    );
    await expect(
      unrotatableService.refreshSession({ refreshToken: "stale-token" }),
    ).rejects.toThrow("Invalid refresh token");

    const revokeFailureTokens: Partial<AuthTokenStore> = {
      revokeRefreshToken: () =>
        errAsync({ code: "token_store_error", message: "offline" }),
    };
    const revokeFailureService = new AuthService(
      missingUsers as AuthUserStore,
      revokeFailureTokens as AuthTokenStore,
    );
    await expect(
      revokeFailureService.revokeRefreshToken({ refreshToken: "token" }),
    ).resolves.toBe(false);
    ...
    await expect(
      new AuthService(
        missingUsers as AuthUserStore,
        revokeSuccessTokens as AuthTokenStore,
    ...
    const users = {
      findByEmail: vi
        ...
        .mockReturnValue(okAsync(activeRecord)),
    };
    ...
    const issueFailureTokens: Partial<AuthTokenStore> = {
      issueUserActionToken: () =>
        errAsync({ code: "token_store_error", message: "issue failed" }),
    };
    await expect(
      new AuthService(
        users as never,
        issueFailureTokens as AuthTokenStore,
      ).issuePasswordResetToken({ email: activeRecord.email }),
    ).resolves.toBeNull();
    await expect(
      actionService.consumeUserActionToken(
        verificationToken ?? "",
        "email_verification",
      ),
    ).resolves.toBe(true);
  ...
  it("maps explicit-tenant preference updates, conflicts, and missing users", async () => {
    ...
    const setPreferences = vi
      ...
      .mockReturnValueOnce(
        errAsync({ code: "repository_error", message: "write failed" }),
      )
      .mockReturnValueOnce(okAsync(null));
    ...
    await expect(
      service.updateUserLocale("preferences-id", DefaultAuthTenantId, "ru-RU"),
    ).resolves.toMatchObject({ locale: "ru" });
  ...
  it("records password methods, handles missing refresh issuance, and maps session signing errors", async () => {
    ...
    await expect(
      new AuthService(
        bootstrapUsers as AuthUserStore,
        undefined,
        undefined,
        resolver as never,
      ).bootstrapUserAccess(createUserRecord({ id: "bootstrap-id" }), ["user"]),
