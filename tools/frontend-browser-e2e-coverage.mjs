...
if (!dist || !appName || !contains || !coverageDir) {
  throw new Error(
    "Usage: frontend-browser-e2e-coverage.mjs --dist <dir> --app-name <name> --contains <text> --coverage-dir <dir>",
  );
}

const root = path.resolve(dist);
const reportDir = path.resolve(coverageDir);
...
async function existingFile(urlPath) {
  ...
  const candidate = requested.startsWith(root)
    ? requested
  ...
  try {
    ...
    // fall back to index.html for app routes
  }
...
const server = createServer(async (request, response) => {
  try {
    const filePath = await existingFile(request.url ?? "/");
...
try {
  ...
  const coveredFiles = coverageMap
    .files()
