...
function assertResolvesTo(dir: string, rel: string, expectedBase: string, label: string): void {
  ...
  assert.strictEqual(
    resolved,
    expected,
    `${label}: "${rel}" from "${dir}" resolves to ${resolved}, expected ${expected}`,
  );
}
...
describe('generated path resolution', () => {
  ...
  it('workspaceTsconfigAliases resolves to config dir for backend app', () => {
    ...
    assertResolvesTo(
      dir,
      `${d}config/vite/workspace-tsconfig-aliases.mjs`,
      'config/vite/workspace-tsconfig-aliases.mjs',
      'backend app vitest aliases',
    );
  });
  ...
  it('fullCoverage resolves to packages dir for backend lib', () => {
    ...
    assertResolvesTo(
      dir,
      `${d}packages/tooling/src/testing/vitest-coverage.mts`,
      'packages/tooling/src/testing/vitest-coverage.mts',
      'backend lib vitest coverage',
    );
  });
  ...
  it('$schema resolves to node_modules/nx for backend app', () => {
    ...
    assertResolvesTo(
      dir,
      `${d}node_modules/nx/schemas/project-schema.json`,
    ...
    for (const dir of existing) {
      ...
      if (fs.existsSync(tcPath)) {
        ...
        assert.strictEqual(
          resolved,
          expected,
          `${dir}/tsconfig.json extends "${extendsPath}" resolves to ${resolved}, expected ${expected}`,
        );
      }
    }
  });
