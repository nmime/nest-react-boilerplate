...
const assertNginxRoutes = (text, { helm = false } = {}) => {
  ...
  before(text, 'location = /admin/ {', 'location ^~ /admin/ {', 'exact /admin/ SPA route precedes /admin API prefix');
  for (const adminSpaRoute of ['/admin/dashboard', '/admin/dashboard/', '/admin/profile', '/admin/profile/']) {
    before(
      text,
      `location = ${adminSpaRoute} {`,
      'location ^~ /admin/ {',
      `${adminSpaRoute} exact SPA route precedes /admin API prefix`,
    );
  }
  assert.ok(
    !text.includes('location ~ ^/admin/(dashboard|profile)/?$'),
    'Admin dashboard/profile SPA routes must use exact locations because ^~ API prefixes skip regex locations.',
  );
