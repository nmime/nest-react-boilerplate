...
describe('AdminAuditLogEntity', () => {
  ...
  it('registers nullable uuid user references without empty-string defaults', () => {
    ...
    expect(AdminAuditLogEntitySchema.meta.properties.actorUserId.default).toBeUndefined();
    expect(AdminAuditLogEntitySchema.meta.properties.targetUserId).toMatchObject({
      type: 'uuid',
      nullable: true,
    });
    expect(AdminAuditLogEntitySchema.meta.properties.targetUserId.default).toBeUndefined();
  });
});
