...
describe('Auth tenant lifecycle entities', () => {
  ...
  it('registers tenant table constraints that mirror lifecycle migrations', () => {
    ...
    expect(metadata.properties.primaryDomain.fieldNames).toContain('primary_domain');
  ...
  it('registers invitation token uniqueness, status check, and lookup indexes', () => {
    ...
    expect(metadata.properties.invitedByUserId.fieldNames).toContain('invited_by_user_id');
    expect(metadata.properties.invitedByUserId.default).toBe('00000000-0000-0000-0000-000000000000');
    expect(metadata.uniques).toContainEqual(
      expect.objectContaining({
