...
test('OTEL NetworkPolicy values reject empty ports and invalid namespace selectors', { skip: !HELM }, () => {
  assert.throws(() => render('nrbtest', ['--set-json', 'networkPolicy.otelCollector.exporterPorts=[]']));
  assert.throws(() =>
    render('nrbtest', ['--set-string', 'networkPolicy.otelCollector.exporterNamespace=Invalid_Name']),
  );
  assert.throws(() =>
    render('nrbtest', ['--set-string', 'networkPolicy.otelCollector.prometheusNamespace=monitoring.example']),
  );

  assert.throws(
    () => render('nrbtest', ['--skip-schema-validation', '--set-json', 'networkPolicy.otelCollector.exporterPorts=[]']),
