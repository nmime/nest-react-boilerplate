...
describe("entity timestamp lifecycle hooks", () => {
  it("drives created-only hooks for join, catalog, and log entities", () => {
    ...
    expect(
      AdminAuditLogEntitySchema.meta.properties.createdAt.onCreate?.(),
    ).toBeInstanceOf(Date);
    expect(
      AuthPermissionEntitySchema.meta.properties.createdAt.onCreate?.(),
    ).toBeInstanceOf(Date);
    expect(
      AuthRolePermissionEntitySchema.meta.properties.createdAt.onCreate?.(),
    ).toBeInstanceOf(Date);
    expect(
      AuthUserRoleEntitySchema.meta.properties.createdAt.onCreate?.(),
    ).toBeInstanceOf(Date);
    expect(
      TransactionalOutboxEventEntitySchema.meta.properties.createdAt.onCreate?.(),
    ).toBeInstanceOf(Date);
  });
  ...
  it("drives tenant lifecycle create/update hooks", () => {
    ...
    for (const schema of [
      ...
      expect(schema.meta.properties.createdAt.onCreate?.()).toBeInstanceOf(
        Date,
      );
      expect(schema.meta.properties.updatedAt.onCreate?.()).toBeInstanceOf(
        Date,
      );
      expect(schema.meta.properties.updatedAt.onUpdate?.()).toBeInstanceOf(
        Date,
      );
    }
  });
...
describe("entity constructor and field-default coverage", () => {
  ...
  it("hydrates role/permission join rows empty for MikroORM", () => {
    expect(new AuthRolePermissionEntity()).toBeInstanceOf(
      AuthRolePermissionEntity,
    );
  });
  ...
  it("honours fully-populated outbox input including explicit timestamps", () => {
    ...
    expect(new TransactionalOutboxEventEntity()).toBeInstanceOf(
      TransactionalOutboxEventEntity,
    );
  });
});
