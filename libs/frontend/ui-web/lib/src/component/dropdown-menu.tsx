...
export const DropdownMenuItem = forwardRef<ComponentRef<typeof DropdownMenuPrimitive.Item>, DropdownMenuItemProps>(
  ({ className, tone = 'default', ...props }, ref) => (
    <DropdownMenuPrimitive.Item
      className={cn('xr-menu__item', tone === 'warning' && 'xr-menu__item--warning', className)}
...
export const DropdownMenuLabel = forwardRef<ComponentRef<typeof DropdownMenuPrimitive.Label>, DropdownMenuLabelProps>(
  ({ className, ...props }, ref) => (
...
export const UiDropdownMenu = ({
  ...
  <DropdownMenu defaultOpen={defaultOpen} onOpenChange={onOpenChange} open={open}>
    <DropdownMenuTrigger asChild>{trigger}</DropdownMenuTrigger>
    <DropdownMenuContent align={align} aria-label={label} className={className} data-admin-primitive="dropdown-menu">
      {items.map((item, index) => (
        <DropdownMenuItem disabled={item.disabled} key={`${index}`} onSelect={item.onSelect} tone={item.tone}>
          {item.label}
        </DropdownMenuItem>
      ))}
    </DropdownMenuContent>
...
export const UiActionsMenu = ({
  ...
  ...props
}: Omit<UiDropdownMenuProps, 'trigger'> & { trigger?: ReactNode }) => <UiDropdownMenu trigger={trigger} {...props} />;
