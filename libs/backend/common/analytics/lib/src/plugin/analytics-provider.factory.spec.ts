...
describe('createAnalyticsProviderPlugins', () => {
  ...
  it('creates an explicit no-op provider', () => {
    expect(createAnalyticsProviderPlugins({ provider: 'noop' })[0]?.name).toBe('noop');
  });

  it('falls back to no-op when an explicit provider is missing credentials', () => {
    expect(createAnalyticsProviderPlugins({ provider: 'posthog' }).map((plugin) => plugin.name)).toEqual(['noop']);
  });
