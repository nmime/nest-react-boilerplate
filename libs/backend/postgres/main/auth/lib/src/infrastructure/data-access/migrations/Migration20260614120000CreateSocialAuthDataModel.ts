...
export class Migration20260614120000CreateSocialAuthDataModel extends Migration {
  override up(): void {
    ...
    this.addSql(
      'create index if not exists "ix__auth_methods__tenant_id_auth_user_id_last_used_at_desc" on "auth_methods" ("tenant_id", "auth_user_id", "last_used_at" desc);',
    );
    this.addSql(
      'create unique index if not exists "uq__auth_methods__tenant_id_auth_user_id" on "auth_methods" ("tenant_id", "auth_user_id") where "method" = \'password\';',
    );
    this.addSql(
      'create unique index if not exists "uq__auth_methods__tenant_id_external_identity_id_method" on "auth_methods" ("tenant_id", "external_identity_id", "method") where "external_identity_id" is not null;',
    );
    this.addSql(
