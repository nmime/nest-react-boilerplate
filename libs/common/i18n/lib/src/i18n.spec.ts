...
function readLocaleCatalogFile(
  ...
  return JSON.parse(readFileSync(join(localeRoot(), locale, fileName), 'utf8')) as Record<string, string>;
}

function readMergedLocale(locale: Locale): Record<string, string> {
  return Object.assign(
    {},
    ...localeCatalogFileNames.map((fileName) => readLocaleCatalogFile(locale, fileName)),
...
describe('@app/common-i18n', () => {
  it('loads backend/runtime translation catalogs from scoped root locale JSON files', () => {
    const expectedFiles = [...localeCatalogFileNames].sort((left, right) => left.localeCompare(right));

    expect(readLocaleCatalogFileNames('en')).toEqual(expect.arrayContaining(expectedFiles));
    expect(readLocaleCatalogFileNames('ru')).toEqual(expect.arrayContaining(expectedFiles));
  ...
  it('keeps locale files thin and free of duplicate raw or merged keys', () => {
    for (const locale of supportedLocales) {
      ...
      for (const fileName of readLocaleCatalogFileNames(locale)) {
        ...
        expect(Object.keys(catalog).length, `${locale}/${fileName} key count`).toBeLessThanOrEqual(
          localeCatalogMaxKeys,
        );
        expect(nonEmptyLines.length, `${locale}/${fileName} line count`).toBeLessThanOrEqual(
          localeCatalogMaxNonEmptyLines,
        );
        expect(duplicateValues(rawKeys), `${locale}/${fileName} raw duplicates`).toEqual([]);
        mergedKeys.push(...Object.keys(catalog));
      }

      expect(duplicateValues(mergedKeys), `${locale} merged duplicates`).toEqual([]);
    }
  });
  ...
  it('keeps every locale JSON catalog in key parity with the fallback catalog', () => {
    const fallbackKeys = Object.keys(translations[fallbackLocale]).sort((left, right) => left.localeCompare(right));

    for (const locale of supportedLocales) {
      expect(
        Object.keys(translations[locale]).sort((left, right) => left.localeCompare(right)),
        locale,
  ...
  it('falls back to the English catalog when a supported locale misses a key', () => {
    const partialTranslations = translations as Record<Locale, Partial<LocaleCatalog>>;
  ...
  it('resolves request locales by query, headers, cookies, and accept-language', () => {
    ...
    expect(resolveLocaleFromRequest({ headers: { 'x-language': 'ru-RU' } })).toBe('ru');
  ...
  it('handles defensive locale parsing and lookup branches', () => {
    ...
    expect(resolveLocaleFromRequest({ headers: { 'x-locale': ['ru'] } })).toBe('ru');
    expect(resolveLocaleFromRequest({ cookies: { lang: 'ru' } })).toBe('ru');
    expect(resolveLocaleFromRequest({ originalUrl: ['ht', 'tp://['].join('') })).toBe('en');
