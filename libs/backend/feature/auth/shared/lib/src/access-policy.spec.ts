...
describe('canonical RBAC ordering', () => {
  it('orders role keys by the system role catalog, appending unknowns', () => {
    expect(orderRoleKeys([AdminRole, 'zeta', UserRole, AdminRole])).toEqual([UserRole, AdminRole, 'zeta']);
  });

  it('orders permission keys by catalog index and appends unknowns alphabetically', () => {
    expect(
      orderPermissionKeys([AdminManageAllPermission, 'zzz:unknown', UserProfileReadPermission, 'aaa:unknown']),
