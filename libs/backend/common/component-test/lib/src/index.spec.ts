...
describe('common component-test exports', () => {
  it('exports Postgres container helpers', () => {
    ...
    expect(componentTest.DockerUnavailableGuardPattern).toBe('const dockerIt = it.skipIf(shouldSkipDockerTest());');
  });
  ...
  it('does not export removed queue container helpers', () => {
    ...
    expect((componentTest as Record<string, unknown>)[removedCreateHelper]).toBeUndefined();
    expect((componentTest as Record<string, unknown>)[removedImageConstant]).toBeUndefined();
  });
});
