...
describe("StaticDataConfigModule", () => {
  it("declares and exports the config service", () => {
    expect(
      Reflect.getMetadata(MODULE_METADATA.PROVIDERS, StaticDataConfigModule),
    ).toContain(StaticDataConfigService);
    expect(
      Reflect.getMetadata(MODULE_METADATA.EXPORTS, StaticDataConfigModule),
    ).toContain(StaticDataConfigService);
  });
});
