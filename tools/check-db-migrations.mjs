...
function columnsFromName(name) {
  return name
    .split(",")
    .map((part) => unquote(part.trim()).replace(/\W+/g, "_").replace(/^_+|_+$/g, ""))
    .filter(Boolean)
    .join("_");
}
...
function validateNoEnums(file, sql) {
  ...
  if (/\bcreate\s+type\b[\s\S]*\bas\s+enum\b/.test(normalized)) {
    fail(file, "PostgreSQL ENUM types are not allowed; use VARCHAR plus a CHECK constraint.");
  }
  if (/\benum\s*\(/.test(normalized)) {
    fail(file, "ENUM-like column types are not allowed; use VARCHAR plus a CHECK constraint.");
  }
}
...
function validateCreateTableColumns(file, sql) {
  ...
  for (const match of sql.matchAll(createTableRegex)) {
    ...
    for (const rawLine of body.split(/\n/)) {
      ...
      if (!line || /^constraint\b/i.test(line) || /^primary\s+key\b/i.test(line) || /^unique\b/i.test(line) || /^check\b/i.test(line)) {
        continue;
      }
      if (!/^"?[a-zA-Z0-9_]+"?\s+/i.test(line)) continue;
...
function validateAlterTableAddColumns(file, sql) {
  const addColumnRegex = /alter\s+table\s+"?([a-zA-Z0-9_]+)"?\s+add\s+column\s+(?:if\s+not\s+exists\s+)?("?[a-zA-Z0-9_]+"?\s+[^;]+);/gi;
  for (const match of sql.matchAll(addColumnRegex)) {
    ...
    if (!/\bnot\s+null\b/i.test(columnDefinition)) {
      fail(file, `ALTER TABLE ${table} ADD COLUMN must define the column as NOT NULL: ${columnDefinition}`);
    }
  }
}
...
function validateOnlineDdlHints(file, sql) {
  ...
  if (!looksMysql) return;

  const ddlRegex = /alter\s+table\s+[^;]+\b(?:add|drop)\s+column\b[^;]*;/gi;
  for (const match of normalized.matchAll(ddlRegex)) {
    const statement = match[0];
    if (!/\balgorithm\s*=\s*instant\b/.test(statement) || !/\block\s*=\s*default\b/.test(statement)) {
      fail(file, "MySQL/MariaDB ADD/DROP COLUMN migrations must include ALGORITHM=INSTANT and LOCK=DEFAULT.");
    }
  }
}

function validateUniqueNames(file, sql) {
  const constraintRegex = /constraint\s+"?([a-zA-Z0-9_]+)"?\s+unique\s*\(([^)]+)\)/gi;
  for (const match of sql.matchAll(constraintRegex)) {
    const actual = match[1];
    const cols = columnsFromName(match[2]);
    const tableMatch = sql.slice(0, match.index).match(/create\s+table\s+(?:if\s+not\s+exists\s+)?"?([a-zA-Z0-9_]+)"?\s*\([^;]*$/i);
    const table = tableMatch?.[1];
    if (table) {
      const expected = `uq__${table}__${cols}`;
      if (actual !== expected) fail(file, `unique constraint must be named ${expected}, got ${actual}`);
    } else if (!/^uq__[a-zA-Z0-9_]+__[a-zA-Z0-9_]+$/.test(actual)) {
      fail(file, `unique constraint name must match uq__{table}__{columns}: ${actual}`);
    }
  }
  ...
  for (const match of sql.matchAll(indexRegex)) {
    ...
    if (actual !== expected) fail(file, `index must be named ${expected}, got ${actual}`);
  }
}

function validateForeignKeyNames(file, sql) {
  const fkRegex = /constraint\s+"?([a-zA-Z0-9_]+)"?\s+foreign\s+key\s*\(([^)]+)\)/gi;
  for (const match of sql.matchAll(fkRegex)) {
    const actual = match[1];
    const column = columnsFromName(match[2]);
    const tableMatch = sql.slice(0, match.index).match(/(?:create|alter)\s+table\s+(?:if\s+not\s+exists\s+)?"?([a-zA-Z0-9_]+)"?[\s\S]*$/i);
    const table = tableMatch?.[1];
    if (!table) {
    ...
    if (actual !== expected) fail(file, `foreign key must be named ${expected}, got ${actual}`);
  }
}
...
const migrationFiles = migrationRoots.flatMap((root) =>
  listFiles(join(repoRoot, root)).filter((file) => /\/migrations\/Migration\d+.*\.ts$/.test(file)),
);
