...
const contextForRequest = (request: RequestWithClientAddress): ExecutionContext =>
  ({
    switchToHttp: () => ({
...
describe('HealthPrivateNetworkIpGuard', () => {
  ...
  it('resolves the client address from the socket when no explicit ip is set', () => {
    expect(guard.canActivate(contextForRequest({ socket: { remoteAddress: privateSocketIp } }))).toBe(true);
  });
