...
describe('parseTmaLaunchState', () => {
  it.each(['link_telegram', 'link_discord', 'link'])('parses %s as a link intent', (startParam) => {
    expect(
      parseTmaLaunchState({
  ...
  it('keeps login intent for normal launch targets', () => {
    expect(parseTmaLaunchState({ isTelegram: true, startParam: 'profile' })).toMatchObject({
  ...
  it('marks unsupported start parameters as not found', () => {
    expect(parseTmaLaunchState({ isTelegram: true, startParam: 'missing' })).toMatchObject({
