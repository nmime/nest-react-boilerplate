...
export class Migration20260517141000AddAuthUserLocale extends Migration {
  override up(): void {
    this.addSql('alter table "auth_users" add column if not exists "locale" varchar(16) not null default \'en\';');
    this.addSql(`
      do $$
      begin
        if not exists (
  ...
  override down(): void {
    this.addSql('alter table "auth_users" drop constraint if exists "ck__auth_users__locale";');
