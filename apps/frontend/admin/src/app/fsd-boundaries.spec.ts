...
const orderedLayers = ['app', 'pages', 'widgets', 'features', 'entities', 'shared'] as const;

type Layer = (typeof orderedLayers)[number];

const layerRank = new Map<Layer, number>(orderedLayers.map((layer, index) => [layer, index]));
...
const getLayer = (path: string): Layer | undefined => orderedLayers.find((layer) => path.startsWith(`../${layer}/`));

const importPattern = /from\s+["']([^"']+)["']|import\s*\(\s*["']([^"']+)["']\s*\)/gu;

const resolveRelativeLayer = (importer: string, specifier: string): Layer | undefined => {
...
describe('admin strict FSD boundaries', () => {
  it('keeps imports directed toward lower layers only', () => {
    ...
    for (const [path, source] of Object.entries(sourceFiles)) {
      ...
      const importerRank = layerRank.get(importerLayer) ?? Number.POSITIVE_INFINITY;
      ...
      for (const match of source.matchAll(importPattern)) {
        ...
        const importedRank = layerRank.get(importedLayer) ?? Number.POSITIVE_INFINITY;
  ...
  it('keeps app imports at provider/router boundary and uses layer public APIs', () => {
    const violations = Object.entries(sourceFiles)
      ...
      .flatMap(([path, source]) => {
        ...
        if (path.startsWith('../app/') && appBusinessBypassPattern.test(source)) {
