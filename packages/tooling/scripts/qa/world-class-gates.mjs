...
import { envList, ensureDir, parseArgs, readJson, run, writeJson } from "./runtime-utils.mjs";
...
async function runGate(name, check) {
  ...
  const started = performance.now();
  try {
    results.push({ name, status: "ok", durationMs: Math.round(performance.now() - started), evidence: await check() });
  } catch (error) {
...
async function loadStressSoak() {
  ...
  assertGate(urls.length > 0, "Load/stress/soak gate requires QA_LOAD_COMMAND or real QA_LOAD_URLS/PERF_URLS", { env: ["QA_LOAD_COMMAND", "QA_LOAD_URLS", "PERF_URLS", "PERF_API_URLS"] });
...
async function chaosResilience() {
  ...
  assertGate(urls.length > 0, "Chaos gate requires real QA_CHAOS_URLS/QA_CANARY_URLS to verify recovery", { env: ["QA_CHAOS_URLS", "QA_CANARY_URLS"] });
  const target = urls[0];
...
async function canarySyntheticMonitoring() {
  ...
  assertGate(urls.length > 0, "Canary/synthetic gate requires real QA_CANARY_URLS/CANARY_URLS", { env: ["QA_CANARY_URLS", "CANARY_URLS"] });
...
const skipped = requiredGates.filter((gate) => selectedGates.size && !selectedGates.has(gate));
