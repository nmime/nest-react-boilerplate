...
describe("parseCidr", () => {
  ...
  it("rejects an address with the wrong number of octets", () => {
    ...
    expect(() => parseCidr("1.2.3.4.5/24")).toThrow(
      "Invalid IPv4 CIDR: 1.2.3.4.5/24",
    );
  });
...
describe("isIpInAllowList edge cases", () => {
  ...
  it("resolves request-based checks via the socket address", () => {
    expect(
      isRequestFromPrivateNetwork({ socket: { remoteAddress: "10.0.0.9" } }),
    ).toBe(true);
    expect(
      isRequestFromPrivateNetwork({ socket: { remoteAddress: "8.8.8.8" } }),
    ).toBe(false);
  });
});
