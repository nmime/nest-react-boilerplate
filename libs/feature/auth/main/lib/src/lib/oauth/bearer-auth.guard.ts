...
export function validateBearerAuthorization(
  ...
  if (!secret) throw new UnauthorizedException("AUTH_JWT_SECRET is not configured.");
...
function extractBearerToken(authorizationHeader: string | undefined): string {
  if (!authorizationHeader) throw new UnauthorizedException("Missing bearer token.");
  const trimmed = authorizationHeader.trim();
  const separatorIndex = trimmed.indexOf(" ");
  if (separatorIndex < 0) throw new UnauthorizedException("Missing bearer token.");
  const scheme = trimmed.slice(0, separatorIndex);
  const token = trimmed.slice(separatorIndex + 1).trim();
  if (scheme.toLowerCase() !== "bearer" || token.length === 0) throw new UnauthorizedException("Missing bearer token.");
  return token;
}

function parseJwt(token: string): { header: JwtHeader; payload: JwtPayload; signingInput: string; signature: string } {
  const parts = token.split(".");
  if (parts.length !== 3 || parts.some((part) => part.length === 0)) throw new UnauthorizedException("Malformed JWT.");
...
function verifyHeader(header: JwtHeader): void {
  if (header.alg === "none") throw new UnauthorizedException("JWT alg none is not allowed.");
  if (!header.alg || !HMAC_ALGORITHMS[header.alg]) throw new UnauthorizedException("Unsupported JWT algorithm.");
}

function verifySignature(header: JwtHeader, signingInput: string, signature: string, secret: string): void {
  const digest = createHmac(HMAC_ALGORITHMS[header.alg as string], secret).update(signingInput).digest();
  const provided = base64UrlDecode(signature);
  if (provided.length !== digest.length || !timingSafeEqual(provided, digest)) throw new UnauthorizedException("Invalid JWT signature.");
}

function verifyTimeClaims(payload: JwtPayload, nowInSeconds: number): void {
  if (typeof payload.exp !== "number" || !Number.isFinite(payload.exp)) throw new UnauthorizedException("JWT expiration is required.");
  if (payload.exp <= nowInSeconds) throw new UnauthorizedException("JWT is expired.");
  if (typeof payload.nbf === "number" && payload.nbf > nowInSeconds) throw new UnauthorizedException("JWT is not active yet.");
}

function verifyIssuer(payload: JwtPayload, expectedIssuer: string | undefined): void {
  if (expectedIssuer && payload.iss !== expectedIssuer) throw new UnauthorizedException("JWT issuer mismatch.");
}
...
function verifyAudience(payload: JwtPayload, expectedAudience: string | undefined): void {
  ...
  if (!matches) throw new UnauthorizedException("JWT audience mismatch.");
}
...
function normalizePrincipalTheme(value: unknown): "system" | "light" | "dark" | undefined {
  if (typeof value !== "string") return undefined;
