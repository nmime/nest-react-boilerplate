...
type AwaitedFunctionReturn<T> = T extends (...args: infer _TArgs) => Promise<infer TResult> ? Awaited<TResult> : never;

export type OpenApiData<T> = NonNullable<Extract<AwaitedFunctionReturn<T>, { data: unknown }>['data']>;

export type OpenApiError<T> = NonNullable<Extract<AwaitedFunctionReturn<T>, { error: unknown }>['error']>;
...
export const isApiClientError = <TError = unknown>(value: unknown): value is ApiClientError<TError> =>
...
export const throwOnOpenApiError = async <TData, TError>(
  resultPromise: Promise<
    { data: TData; error?: never; response: Response } | { data?: never; error: TError; response: Response }
  >,
): Promise<NonNullable<TData>> => {
  const result = await resultPromise;
  if (result.error !== undefined) {
    throw new ApiClientError<TError>(result.response.status, result.error, result.response);
  }
...
export const throwOnOpenApiErrorData = async <TData, TError>(
  resultPromise: Promise<
    { data: TData; error?: never; response: Response } | { data?: never; error: TError; response: Response }
  >,
): Promise<EnvelopeData<TData>> => unwrapEnvelopeData(await throwOnOpenApiError(resultPromise));
