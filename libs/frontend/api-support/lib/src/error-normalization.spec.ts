...
describe("normalizeApiError", () => {
  ...
  it("classifies 401 and 403 responses as auth errors", () => {
    expect(
      normalizeApiError({ response: { status: 401, statusText: "" } }).kind,
    ).toBe("auth");
    expect(
      normalizeApiError({ response: { status: 403, statusText: "" } }).kind,
    ).toBe("auth");
  });
  ...
  it("classifies 5xx as server and other 4xx as client errors", () => {
    expect(
      normalizeApiError({ response: { status: 503, statusText: "" } }).kind,
    ).toBe("server");
    expect(
      normalizeApiError({ response: { status: 404, statusText: "" } }).kind,
    ).toBe("client");
  });

  it("falls back to the unknown kind for non-error status codes", () => {
    expect(
      normalizeApiError({ response: { status: 302, statusText: "" } }).kind,
    ).toBe("unknown");
  });
  ...
  it("prefers explicit body codes over the http status code", () => {
    ...
    expect(
      normalizeApiError({ response: { status: 404, statusText: "" } }).code,
    ).toBe("http.404");
  });
  ...
  it("derives a human message from body fields, error, or status text", () => {
    ...
    expect(
      normalizeApiError({ response: { status: 500, statusText: "" } }).message,
...
describe("isNetworkFailure", () => {
  ...
  it("ignores unrelated errors and non-errors", () => {
    expect(isNetworkFailure(new Error("validation failed on server"))).toBe(
      false,
    );
...
describe("readJsonBody", () => {
  it("skips bodies that are not declared JSON", async () => {
    await expect(
      readJsonBody(jsonResponse("<html></html>", { contentType: "text/html" })),
  ...
  it("swallows malformed JSON bodies", async () => {
    await expect(
      readJsonBody(jsonResponse("{not json", {})),
    ).resolves.toBeUndefined();
  });
...
describe("enrichJsonResponse", () => {
  it("merges the frontend error into record bodies", async () => {
    ...
    const enriched = await enrichJsonResponse(
      jsonResponse({ code: "boom", detail: "kept" }),
      normalized,
    );
  ...
  it("wraps non-record bodies in a fresh frontend-error envelope", async () => {
    ...
    const enriched = await enrichJsonResponse(
      jsonResponse("plain text", { contentType: "text/plain" }),
      normalized,
    );
    const body = (await enriched.json()) as Record<string, unknown>;
