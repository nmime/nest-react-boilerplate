  ...
  it('uses the Next.js-compatible value first', () => {
    expect(
      resolveBetterAuthBaseUrl({
        ...
        VITE_API_BASE_URL: 'https://vite-auth.example.com',
      }),
    ).toBe('https://auth.example.com');
  });

  it('uses the Vite-compatible value when available', () => {
    expect(resolveBetterAuthBaseUrl({ VITE_API_BASE_URL: 'https://vite-auth.example.com' })).toBe(
      'https://vite-auth.example.com',
    );
  });
