...
export function readBoolean(value: string | undefined, name = 'boolean value'): boolean | undefined {
...
export function readSslRejectUnauthorized(env: { POSTGRES_SSL_REJECT_UNAUTHORIZED?: string | boolean }): boolean {
  try {
    ...
    if (error instanceof Error) {
      throw new Error('POSTGRES_SSL_REJECT_UNAUTHORIZED must be a boolean value.', { cause: error });
    }
