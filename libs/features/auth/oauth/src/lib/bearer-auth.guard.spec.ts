...
const signingKey = createHash("sha256")
  .update("features-auth-oauth-jwt-tests")
  .digest("hex");
...
describe("BearerAuthGuard", () => {
  ...
  it("attaches principal to user and auth request fields", () => {
    ...
    process.env.AUTH_JWT_SECRET = signingKey;
    try {
  ...
  it.each([
    ...
    [
      ...
      { AUTH_JWT_ISSUER: "issuer", AUTH_JWT_SECRET: signingKey },
      "issuer mismatch",
    ],
    [
      "audience mismatch",
      `Bearer ${signToken({ aud: "web", sub: "user-id" })}`,
      { AUTH_JWT_AUDIENCE: "api", AUTH_JWT_SECRET: signingKey },
      "audience mismatch",
    ],
