# @app/frontend-feature-shared-preferences Instructions
...
- Keep this platform-neutral: no `react-dom` / `react-native` / platform UI imports, so web, native, and admin can all consume it.
- This library must stay product-agnostic: no `scope:user`- or `scope:admin`-specific imports. Callers inject product specifics (profile query keys, override behavior) through options.
