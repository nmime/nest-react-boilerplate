...
export function readTenantIdHeader(request: AuthenticatedRequest): string | undefined {
...
export function assertRequestTenantMatchesPrincipal(
  ...
  const normalizedRequestedTenantId = requestedTenantId ? normalizeTenantId(requestedTenantId) : undefined;

  if (requestedTenantId && !normalizedRequestedTenantId) {
    throw new UnauthorizedException('Invalid tenant id.');
  }
  if (normalizedRequestedTenantId && normalizedRequestedTenantId !== principal.tenantId) {
