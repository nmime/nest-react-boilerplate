...
describe('shadcn/ui frontend foundation', () => {
  it('maps shadcn semantic tokens onto existing theme variables', () => {
    ...
    expect(styles).toMatch(/:root\[data-theme=['"]light['"]\]/u);
    expect(styles).toMatch(/:root\[data-theme=['"]dark['"]\]/u);
  ...
  it('keeps the shadcn registry pointed at the public web UI package', () => {
