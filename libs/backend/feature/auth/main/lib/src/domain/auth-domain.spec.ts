...
describe("auth domain services", () => {
  ...
  it("hashes and verifies password credentials", () => {
    ...
    expect(verifyPassword("password123", "pbkdf2_sha256$$salt$digest")).toBe(
      false,
    );
    const from = vi.spyOn(Buffer, "from").mockImplementationOnce(() => {
      throw new Error("decode failed");
    });
    expect(verifyPassword("password123", "pbkdf2_sha256$1$salt$digest")).toBe(
      false,
    );
    from.mockRestore();
  });
  ...
  it("parses tenant ids and reports invalid tenants as domain errors", () => {
    expect(parseDomainTenantId("11111111-1111-4111-8111-111111111111")).toBe(
      "11111111-1111-4111-8111-111111111111",
    );
    expect(() => parseDomainTenantId("not valid")).toThrow(
      InvalidAuthTenantIdError,
    );
  });

  it("signs JWTs and reports missing configuration as a domain error", () => {
    expect(() => signDomainJwt({ sub: "user" }, {}, 60)).toThrow(
      AuthJwtSigningError,
    );
    expect(
      signDomainJwt(
