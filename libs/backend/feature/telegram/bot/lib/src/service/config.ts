...
import type { TelegramBotConfig } from '../type';

const DefaultSessionTtlSeconds = 60 * 60 * 24 * 14;
const DefaultRateLimitTimeFrameMs = 1_000;
const DefaultRateLimit = 3;

export function resolveTelegramBotConfig(env: NodeJS.ProcessEnv = process.env): TelegramBotConfig {
  ...
  return {
    ...
    rateLimit: {
      timeFrameMs: readPositiveInt(env.TELEGRAM_BOT_RATE_LIMIT_WINDOW_MS, DefaultRateLimitTimeFrameMs),
...
export function assertPollingRuntimeAllowed(config: Pick<TelegramBotConfig, 'mode' | 'environment'>): void {
  if (config.mode === 'webhook') {
    throw new Error('Telegram polling runtime cannot start when TELEGRAM_BOT_MODE=webhook.');
  }
...
export function signStartPayload(value: string, secret: string, now = Math.floor(Date.now() / 1000)): string {
...
function normalizePathname(pathname: string): string {
  return pathname.endsWith('/') && pathname !== '/' ? pathname.slice(0, -1).toLowerCase() : pathname.toLowerCase();
}
