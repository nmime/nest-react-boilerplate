...
describe('API resilience middleware', () => {
  it('wraps generated-client fetches with offline, auth-required, toast, and enriched error handling', async () => {
    ...
    expect(authBody[FrontendErrorKey]).toMatchObject({
      kind: 'auth',
...
describe('auth refresh middleware', () => {
  it('shares one refresh promise across parallel 401s and retries with the new token', async () => {
    const eventHub = createApiRuntimeEventHub();
    const refreshAccessToken = vi.fn<() => Promise<string>>().mockImplementation(
      () =>
        new Promise((resolve) =>
          setTimeout(() => {
            resolve('fresh');
    ...
    const fetchMock = vi
      .fn<typeof fetch>()
      .mockResolvedValue(new Response(JSON.stringify({ data: true }), { status: 200 }));
    ...
    const requestA = await invokeOnRequest(middleware, new Request('https://api.example.test/a'));
    const requestB = await invokeOnRequest(middleware, new Request('https://api.example.test/b'));
    ...
    expect((fetchMock.mock.calls[0]?.[0] as Request).headers.get('Authorization')).toBe('Bearer fresh');
  });
  ...
  it('clears auth and returns the original 401 when the fetch wrapper cannot refresh', async () => {
    const clearAuth = vi.fn<() => void>();
    const baseFetch = vi.fn<typeof fetch>().mockResolvedValue(new Response(null, { status: 401 }));
