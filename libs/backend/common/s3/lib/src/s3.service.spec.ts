...
describe("S3Service", () => {
  it("delegates put/get/list/delete to the underlying client", async () => {
    ...
    await expect(
      service.getObject({ bucket: "media", key: "a" }),
    ).resolves.toMatchObject({ key: "a" });
    await expect(
      service.listObjects({ bucket: "media" }),
  ...
  it.each([
    [
      "putObject",
      (service: S3Service) =>
        service.putObject({ bucket: "b", key: "k", body: "v" }),
    ],
    [
      "getObject",
      (service: S3Service) => service.getObject({ bucket: "b", key: "k" }),
    ],
    [
      "deleteObject",
      (service: S3Service) => service.deleteObject({ bucket: "b", key: "k" }),
    ],
    [
      "listObjects",
      (service: S3Service) => service.listObjects({ bucket: "b" }),
    ],
    ...
    async (operation, invoke) => {
      ...
      expect((error as Error).message).toBe(
        `Object storage operation failed: ${operation}`,
      );
      expect((error as ObjectStorageOperationFailedException).cause).toBe(
        cause,
      );
    },
  );
