...
export class RedisConfigService {
  protected readonly configService = createConfig<RedisEnvironment>(redisEnvSchema);

  constructor(private readonly options: RedisConfig = {}) {}

  get mode(): RedisMode {
    return this.options.mode === undefined ? this.configService.get('REDIS_MODE') : toRedisMode(this.options.mode);
  }
  ...
  get sentinelGroupIdentifier(): string | undefined {
    return this.options.sentinelGroupIdentifier ?? this.configService.get('REDIS_SENTINEL_GROUP_IDENTIFIER');
  }
  ...
  get lazyConnect(): boolean {
    return this.options.lazyConnect ?? this.configService.get('REDIS_LAZY_CONNECT');
  }
