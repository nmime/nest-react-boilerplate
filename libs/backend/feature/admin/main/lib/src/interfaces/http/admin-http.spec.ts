...
describe("toHttpException", () => {
  it("maps not_found to NotFoundException", () => {
    expect(() =>
      toHttpException(new AdminApplicationError("not_found", "missing")),
    ).toThrow(NotFoundException);
  });

  it("maps conflict to ConflictException", () => {
    expect(() =>
      toHttpException(new AdminApplicationError("conflict", "dup")),
    ).toThrow(ConflictException);
  });

  it("maps invalid_access_policy to BadRequestException", () => {
    expect(() =>
      toHttpException(new AdminApplicationError("invalid_access_policy", "no")),
  ...
  it("maps sensitive_policy_violation to BadRequestException", () => {
    expect(() =>
      toHttpException(
        new AdminApplicationError("sensitive_policy_violation", "no"),
      ),
    ).toThrow(BadRequestException);
  });
  ...
  it("maps other application error codes to InternalServerErrorException", () => {
    expect(() =>
      toHttpException(new AdminApplicationError("repository_error", "boom")),
...
describe("executeAdminUseCase", () => {
  it("returns the handler result on success", async () => {
    await expect(
      executeAdminUseCase(() => Promise.resolve("ok")),
    ).resolves.toBe("ok");
  });
  ...
  it("translates a thrown application error into an HTTP exception", async () => {
    await expect(
      executeAdminUseCase(() =>
        Promise.reject(new AdminApplicationError("not_found", "missing")),
      ),
...
describe("requestContextFromRequest", () => {
  it("reads the first value of an array header", () => {
    expect(requestContextFromRequest(requestWith(["first", "second"]))).toEqual(
      { requestId: "first" },
    );
  });
