...
describe("UiDataTable branch coverage", () => {
  ...
  it("applies center and right alignment classes to headers and cells", () => {
    ...
    expect(
      screen.getByRole("columnheader", { name: "Centered" }).className,
    ).toContain("text-center");
    expect(
      screen.getByRole("columnheader", { name: "Righted" }).className,
    ).toContain("text-right");
    expect(screen.getByRole("cell", { name: "Ada" }).className).toContain(
      "text-center",
    );
    expect(screen.getByRole("cell", { name: "usr_1" }).className).toContain(
      "text-right",
    );
  });
  ...
  it("renders a React node error without a string description", () => {
    ...
    expect(screen.getByTestId("node-error").textContent).toBe(
      "Broken pipeline",
    );
  });
});
