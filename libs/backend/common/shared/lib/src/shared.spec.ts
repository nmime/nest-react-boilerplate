...
import { isNonEmptyString, maskSecret, normalizeStringList, readRequiredEnv, uniqueStrings } from './index';

describe('@app/backend-common-shared', () => {
  it('deduplicates strings while preserving order', () => {
    expect(uniqueStrings(['admin', 'admin', 'support'])).toEqual(['admin', 'support']);
  });
  ...
  it('reads required environment variables and rejects missing values', () => {
    ...
    expect(() => readRequiredEnv({ TOKEN: ' ' }, 'TOKEN')).toThrow('TOKEN is required.');
  });
