...
describe("problemCodeForStatus", () => {
  it("maps known statuses to their canonical short codes", () => {
    ...
    expect(problemCodeForStatus(HttpStatus.TOO_MANY_REQUESTS)).toBe(
      "rate-limited",
    );
    expect(problemCodeForStatus(HttpStatus.INTERNAL_SERVER_ERROR)).toBe(
      "internal-server-error",
    );
  });

  it("derives a slug from the title for unmapped statuses", () => {
    expect(problemCodeForStatus(HttpStatus.I_AM_A_TEAPOT)).toBe(
      "i-am-a-teapot",
    );
    expect(problemCodeForStatus(HttpStatus.UNPROCESSABLE_ENTITY)).toBe(
