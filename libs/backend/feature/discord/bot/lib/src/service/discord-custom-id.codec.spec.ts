...
function forge(bodyParts: string[]): string {
  ...
  const signature = createHmac("sha256", testSigningKey)
    .update(body)
    .digest()
    .subarray(0, 16)
    .toString("base64url");
...
describe("DiscordCustomIdCodec", () => {
  ...
  it("rejects tampered, expired, and mismatched component ids", () => {
    ...
    expect(() => {
      codec.assertOwner(
        codec.decode(customId, { secret: testSigningKey, now }),
        {
          userId: "999",
          guildId: "234567890123456789",
          tenantId: "00000000-0000-0000-0000-000000000000",
  ...
  it("keeps every supported action within length budget", () => {
    ...
    for (const action of actions) {
      ...
      expect(
        codec.decode(customId, { secret: testSigningKey, now }).action,
      ).toBe(action);
    }
  });
  ...
  it("rejects malformed, missing nonce, and oversized custom ids", () => {
    ...
    expect(() =>
      codec.decode("nrb:1:h", { secret: testSigningKey, now }),
    ).toThrow(/tampered/u);
    expect(() =>
      codec.decode(missingNonce.join(":"), { secret: testSigningKey, now }),
    ).toThrow();
    expect(() =>
      codec.encode(
        {
  ...
  it("rejects validly-signed ids with the wrong prefix or version", () => {
    ...
    expect(() =>
      codec.decode(
        forge(["bad", "1", "h", "nonce", "1", "-", "0", "1", "zzzzzzz"]),
        { secret: testSigningKey, now },
      ),
    ).toThrow(/tampered/u);
    expect(() =>
      codec.decode(
        forge(["nrb", "2", "h", "nonce", "1", "-", "0", "1", "zzzzzzz"]),
        { secret: testSigningKey, now },
      ),
    ).toThrow(/tampered/u);
  });
  ...
  it("rejects signatures whose decoded length cannot match", () => {
    ...
    expect(() =>
      codec.decode(parts.join(":"), { secret: testSigningKey, now }),
    ).toThrow(/tampered/u);
  });

  it("rejects validly-signed ids with non-numeric issued/expiry stamps", () => {
    const codec = new DiscordCustomIdCodec();
    const customId = forge([
      "nrb",
      "1",
      "h",
      "nonce",
