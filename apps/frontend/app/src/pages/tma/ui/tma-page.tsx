...
export function TmaPage({ fallbackStartParam, navigate }: Readonly<TmaPageProps>) {
  ...
  return (
    <UiSection className="xr-tma-section" eyebrow="Telegram" title="Telegram Mini App access">
      <div className="xr-tma-layout" data-design-marker="tma-v3">
        <UiCard className="xr-tma-intro xr-surface-glow" title="Mini App handoff">
          <div className="xr-card-stack">
            <UiAlert className="xr-inline-alert" tone="info">
              <strong>{state.intent === 'link' ? t('tma.link.required') : t('tma.idle')}</strong>
              <span>Deep links, Telegram launch data, and fallback routes share one verification surface.</span>
            </UiAlert>
        ...
        <div className="xr-stat-grid xr-stat-grid--compact">
          ...
          <UiStatCard detail="/link/telegram" label="Link route" value="ready" />
          <UiStatCard detail={state.deepNavigationState} label="Deep link" value={state.status} />
        </div>
        <TmaAuthPanel
