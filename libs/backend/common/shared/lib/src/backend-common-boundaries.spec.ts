...
const forbiddenImports = [
  ...
  ['backend', 'admin'].join('/'),
  ['backend', 'auth'].join('/'),
  ['backend', 'discord'].join('/'),
  ['backend', 'feature-flags'].join('/'),
  ['backend', 'postgres'].join('/'),
  ['backend', 'telegram'].join('/'),
  ['backend', 'user'].join('/'),
  ['libs', 'frontend'].join('/'),
];

const importSpecifierPattern = /\b(?:from\s+|import\s*\(\s*)["'](?<specifier>[^"']+)["']/gu;
...
describe('backend common clean boundaries', () => {
  ...
  it('classifies every backend common library with a clean-boundary role', () => {
    for (const [library, boundaryTag] of Object.entries(expectedProjects)) {
      ...
      expect(project.tags).toEqual(expect.arrayContaining(['platform:backend', 'scope:shared', boundaryTag]));
    }
  });
  ...
  it('keeps backend common free of feature, persistence, bot, and frontend imports', () => {
    const violations = listTypeScriptFiles(commonRoot).flatMap((filePath) => {
      ...
      const imports = Array.from(source.matchAll(importSpecifierPattern)).map((match) => match.groups?.specifier ?? '');
