...
const sourceRoots = ['apps/frontend', 'libs/frontend'].map((root) => join(workspaceRoot, root));
...
const toWorkspacePath = (path: string): string => relative(workspaceRoot, path).split(sep).join('/');
...
describe('shadcn/ui monorepo boundary', () => {
    ...
    const duplicateUiDirectories = sourcePaths
      ...
      .filter(
        (path) =>
          ...
          (path.endsWith('/components/ui') || path.endsWith('/src/components/ui')),
      );
  ...
  it('keeps Radix and shadcn helper dependencies behind the shared web UI facade', () => {
    const directRuntimeImports = sourceFiles
      .filter((path) => !toWorkspacePath(path).startsWith('libs/frontend/ui-web/'))
  ...
  it('keeps web shadcn UI and native Tamagui UI on separate renderer boundaries', () => {
    const webUiInNative = sourceFiles
      .filter((path) => {
        ...
        return (
          workspacePath.startsWith('apps/frontend/mobile/') || workspacePath.startsWith('libs/frontend/ui-native/')
        );
      })
    ...
    const nativeUiInWeb = sourceFiles
      ...
      .filter(
        (path) => readSource(path).includes('@app/frontend-ui-native') || readSource(path).includes('from "tamagui"'),
      )
      .map(toWorkspacePath);
  ...
  it('keeps the shadcn icon library backed by its runtime dependency', () => {
    const componentsJson = JSON.parse(readFileSync(join(workspaceRoot, 'components.json'), 'utf8')) as {
      iconLibrary?: string;
    };
    const frontendPackageJson = JSON.parse(readFileSync(join(workspaceRoot, 'libs/frontend/package.json'), 'utf8')) as {
