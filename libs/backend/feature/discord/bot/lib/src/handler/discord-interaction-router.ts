...
type RoutedContext = Required<Pick<DiscordInteractionContext, 'customIdSecret'>> & {
...
export class DiscordInteractionRouter {
  ...
  async route(interaction: APIInteraction, context: DiscordInteractionContext): Promise<APIInteractionResponse> {
    ...
    if (interaction.type === InteractionType.ApplicationCommand) {
      return this.routeCommand(interaction as ChatInputInteraction, routedContext);
    }
  ...
  private routeModal(_interaction: APIModalSubmitInteraction, context: { locale: string }): APIInteractionResponse {
    return ephemeral(t('bot.error.unknown', context.locale));
  }
  ...
  private async link(
    ...
    try {
      ...
      return respond(mode, t('discord.commands.link.message', context.locale), [
        createButtonRow([
          linkButton(t('discord.components.linkButton', context.locale), result.authorizationUrl),
          this.stateButton('cancel', t('discord.components.cancelButton', context.locale), interaction, context),
        ]),
      ]);
    } catch (error) {
      const message =
      ...
      return respond(mode, message, [
        createButtonRow([
          this.stateButton('back', t('discord.components.backButton', context.locale), interaction, context),
          this.stateButton('home', t('discord.components.homeButton', context.locale), interaction, context),
        ]),
      ]);
    }
  }
    ...
    return respond(mode, message, [
      createButtonRow([
        this.stateButton(
          ...
          t(status.linked ? 'discord.components.unlinkButton' : 'discord.components.linkButton', context.locale),
          interaction,
          context,
  ...
  private openApp(context: RoutedContext): APIInteractionResponse {
    if (!context.webAppBaseUrl) {
      return ephemeral(t('discord.messages.openAppUnavailable', context.locale));
    }
  ...
  private help(
    ...
    const buttons: ReturnType<typeof linkButton | typeof customButton>[] = [
      ...
      this.stateButton('back', t('discord.components.statusButton', context.locale), interaction, context),
      this.stateButton('home', t('discord.components.homeButton', context.locale), interaction, context),
    ];
    if (context.webAppBaseUrl) {
      buttons.push(linkButton(t('discord.components.openAppButton', context.locale), context.webAppBaseUrl));
    } else {
      buttons.push(
        this.stateButton('open_app', t('discord.components.openAppButton', context.locale), interaction, context),
      );
    }
    return respond(mode, t('discord.commands.help.message', context.locale), [createButtonRow(buttons)]);
  }
  ...
  private stateButton(
    ...
    context: RoutedContext,
    style:
      ButtonStyle.Primary | ButtonStyle.Secondary | ButtonStyle.Success | ButtonStyle.Danger = ButtonStyle.Secondary,
  ) {
    const user = interactionUser(interaction);
