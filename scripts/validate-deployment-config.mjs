...
const has = (text, needle, label = needle) =>
  assert.ok(
    ...
    `Missing expected deployment config: ${label}`,
  );
...
has(
  ...
  "dev frontend healthcheck targets container port 8080",
);
has(
  devCompose,
...
has(
  ...
  "dev Compose backend defaults to development NODE_ENV",
);
has(
  devBackendEnv,
...
const jwtSecretDefault = devBackendEnv.match(
...
assert.ok(
  envExampleJwtSecret,
  "Missing .env.example AUTH_JWT_SECRET placeholder",
);
assert.ok(
...
has(
  ...
  "production env example forces shared Redis rate limiting",
);
has(
  productionEnvExample,
...
has(
  ...
  "production Compose points APIs at Redis",
);
has(
  prodBackendEnv,
  "REDIS_KEY_PREFIX: ${REDIS_KEY_PREFIX:-nrb:}",
  "production Compose sets Redis key prefix",
);
const prodBackendService = section(
  prodCompose,
  "x-backend-service:",
  "\nx-frontend-service:",
);
has(
  prodBackendService,
  "redis:",
  "production backend services depend on Redis",
);
has(
  prodBackendService,
  "condition: service_healthy",
  "production backend services wait for healthy dependencies",
);
const prodRedisService = section(prodCompose, "  redis:", "\n\n  migrate:");
has(
  prodRedisService,
  "image: redis:7-alpine",
  "production Compose Redis image",
);
has(
  prodRedisService,
  "redis-server",
  "production Compose starts Redis server explicitly",
);
has(
  prodRedisService,
  "redis-cli",
  "production Compose Redis healthcheck command",
);
...
assert.ok(
  smokeJwtSecretDefault,
  "Docker smoke script must set an AUTH_JWT_SECRET default",
);
assert.ok(
...
const assertNginxRoutes = (text, { helm = false } = {}) => {
  has(
    text,
    helm
      ? "listen {{ default 8080 .Values.frontendNginx.listenPort }};"
      : "listen 8080;",
    "frontend nginx listen port",
  );
  has(
    text,
    helm ? ".Values.frontendNginx.healthPath" : "/nginx-health",
    "nginx health route",
  );
  before(
    text,
    "location = /admin {",
    "location ^~ /admin/ {",
    "exact /admin SPA route precedes /admin API prefix",
  );
  before(
    text,
    "location = /admin/ {",
  ...
  has(
    text,
    "location ^~ /auth/",
    "auth API prefix route cannot be shadowed by regex static assets",
  );
  has(
    text,
    "location ^~ /profile/",
    "profile/user API prefix route cannot be shadowed by regex static assets",
  );
  has(
    text,
    "location ^~ /admin/",
