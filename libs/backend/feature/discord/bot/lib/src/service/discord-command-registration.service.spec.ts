...
describe('Discord command registration', () => {
  ...
  it('keeps command names lowercase and descriptions within Discord limits', () => {
    for (const { json } of buildDiscordCommands()) {
      ...
      for (const [locale, value] of Object.entries(json.name_localizations ?? {})) {
      ...
      for (const option of json.options ?? []) {
        ...
        for (const [locale, value] of Object.entries(option.name_localizations ?? {})) {
  ...
  it('registers guild commands with native fetch URL/body and no real network', async () => {
    ...
    expect((fetchMock.mock.calls[0]?.[1] as { body?: string } | undefined)?.body).toBe(
  ...
  it('registers global commands and reports Discord failures', async () => {
    ...
    expect(fetchMock.mock.calls[0]?.[0]).toBe('https://discord.com/api/v10/applications/123456789012345678/commands');
  });
  ...
  it('reads the registration snapshot from process.env by default', () => {
    vi.stubEnv('DISCORD_APPLICATION_ID', '123456789012345678');
    vi.stubEnv('DISCORD_PUBLIC_KEY', 'a'.repeat(64));
    vi.stubEnv('DISCORD_CUSTOM_ID_SECRET', 'test-key');
    const service = new DiscordCommandRegistrationService(new DiscordBotConfig());

    expect(service.dryRun()).toMatchObject({
      scope: 'global',
      applicationId: '123456789012345678',
    });
  });

  it('uses a global route when guild scope is set without a guild id', async () => {
    const fetchMock = vi.fn().mockResolvedValue({ ok: true, status: 200 });
    vi.stubGlobal('fetch', fetchMock);
    const service = new DiscordCommandRegistrationService(new DiscordBotConfig());
    ...
    await service.register({
      ...baseEnv,
      DISCORD_BOT_TOKEN: ['opaque', 'guild', 'credential'].join('-'),
      DISCORD_COMMAND_REGISTRATION_SCOPE: 'guild',
    });
