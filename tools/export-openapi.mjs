...
function parseArgs(argv) {
  const args = {
    app: "auth-app-api",
    output: "docs/openapi/auth-app-api.json",
    port: "3999",
    dryRun: false,
  };
...
async function main() {
  ...
  if (args.help) {
    console.log(
      "Usage: pnpm api:openapi -- [--app auth-app-api] [--output docs/openapi/auth-app-api.json] [--dry-run]",
    );
    return;
  }
  const env = {
  ...
  if (args.dryRun) {
    console.log(
      JSON.stringify(
        { status: "dry-run", command, url, output: args.output },
        null,
        2,
      ),
    );
  ...
  try {
    ...
    mkdirSync(args.output.slice(0, args.output.lastIndexOf("/")) || ".", {
      recursive: true,
    });
    writeFileSync(
      args.output,
