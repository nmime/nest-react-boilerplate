...
Keep the payment lifecycle — its seven states, its single transition
function, its exact money, its immutable FX snapshot, its idempotent
creation, and its expiry and underpaid closure — explicit and enforced
identically by every path that can move a payment. The five payments
projects carry these requirements together with the provider and webhook
capabilities; the scaffold suites that exist at this stage of the change
are the scaffold-level evidence the sidecars point at, re-pointed from the
retired scaffold requirement. Units U2–U9 replace the sidecar evidence
with the real suites as they land.
