...
const buttons = (response: APIInteractionResponse) =>
  ((
    response.data as
      { components?: Array<{ components: unknown[] }> } | undefined
...
describe("DiscordInteractionRouter", () => {
  ...
  it("returns an actionable error when status lookup fails", async () => {
    ...
    const response = await harness(account).router.route(
      command("account", "status"),
      context(),
    );
  ...
  it("unlinks the account when the confirm action succeeds", async () => {
    ...
    const response = await setup.router.route(
      component(validComponentId(setup, "confirm")),
      context(),
    );
    ...
    expect(unlinkProviderIdentity).toHaveBeenCalledWith(
      "identity-1",
  ...
  it("maps unexpected component validation errors to an expired-action message", () => {
    expect(componentValidationMessage(new Error("boom"), "en")).toBe(
  ...
  it("rejects an interaction that carries no Discord user", async () => {
    await expect(
      harness().router.route(
        command("help", undefined, { user: undefined }),
        context(),
      ),
    ).rejects.toThrow("Discord interaction user missing.");
  });
});
