...
export function installFixedSystemTime(baseTime: Date = new Date('2025-01-01T00:00:00Z')): number {
  if (fakeTimerActive) {
    throw new Error(
...
export function advanceFixedSystemTime(ms: number): void {
  if (!fakeTimerActive) {
    throw new Error('No fixed system time is installed. Call installFixedSystemTime() first.');
  }
  if (ms >= 0) {
...
export async function withFixedSystemTime<T>(baseTime: Date, fn: () => T | Promise<T>): Promise<T> {
