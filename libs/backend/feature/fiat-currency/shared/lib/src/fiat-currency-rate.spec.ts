import { Money, MoneyCurrencyMismatchError } from '@app/common-money';
...
describe('convertFiatMoney', () => {
  it('converts a zero-exponent currency up to USD cents', () => {
    ...
    expect(convertFiatMoney(yen, { code: 'JPY', usdPerUnit: '0.0064' }, usdRateQuote)).toEqual(Money.of(640, 'USD'));
  });

  it('converts USD cents down to a zero-exponent currency', () => {
    const dollars = Money.of(10_000, 'USD');
  ...
  it('converts between two non-USD currencies through their USD rates', () => {
    const euros = Money.of(10_000, 'EUR');
  ...
  it('refuses an amount that is not in the source currency', () => {
    expect(() => convertFiatMoney(Money.of(100, 'USD'), { code: 'EUR', usdPerUnit: '1.08' }, usdRateQuote)).toThrow(
...
describe('fiatMoneyToUsd', () => {
  it('quotes an amount in USD', () => {
    expect(fiatMoneyToUsd(Money.of(10_000, 'EUR'), { code: 'EUR', usdPerUnit: '1.08' })).toEqual(
      Money.of(10_800, 'USD'),
    );
  });
});
...
describe('fiatMoneyFromUsd', () => {
  it('prices a USD amount in another currency', () => {
    expect(fiatMoneyFromUsd(Money.of(10_800, 'USD'), { code: 'EUR', usdPerUnit: '1.08' })).toEqual(
      Money.of(10_000, 'EUR'),
    );
  });
});
