...
export class NotificationHealthConfigService implements NotificationHealthConfig {
  ...
  get alertIntervalMinutes(): number {
    return this.configService.get<number>('NOTIFICATION_HEALTH_ALERT_INTERVAL_MINUTES', 30);
  }

  get errorThreshold(): number {
    return this.configService.get<number>('NOTIFICATION_HEALTH_ERROR_THRESHOLD', 0);
  }
}
