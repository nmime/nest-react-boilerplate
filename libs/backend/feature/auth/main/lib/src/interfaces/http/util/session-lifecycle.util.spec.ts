...
describe("session lifecycle utilities", () => {
  it("handles absent, promise, void, and callback-style session methods", async () => {
    ...
    await expect(
      callSessionMethod({ session: { save: promiseSave } }, "save"),
    ).resolves.toBeUndefined();
    expect(promiseSave).toHaveBeenCalledOnce();

    const voidSave = vi.fn(() => undefined);
    await expect(
      callSessionMethod({ session: { save: voidSave } }, "save"),
    ).resolves.toBeUndefined();
    ...
    await expect(
      callSessionMethod({ session: { destroy: callbackDestroy } }, "destroy"),
    ).rejects.toThrow("Session lifecycle method failed.");

    const errorDestroy = vi.fn((callback: (error?: unknown) => void) => {
      callback(new Error("destroy failed"));
    });
    await expect(
      callSessionMethod({ session: { destroy: errorDestroy } }, "destroy"),
