...
describe("OpenTelemetry bootstrap", () => {
  ...
  it("resolves enablement, endpoints, and headers from standard OTEL env vars", () => {
    ...
    expect(
      isOpenTelemetryEnabled(
        {},
        { OTEL_EXPORTER_OTLP_TRACES_ENDPOINT: "https://t" },
      ),
    ).toBe(true);
    ...
    expect(
      resolveOtlpEndpoint(
        { OTEL_EXPORTER_OTLP_TRACES_ENDPOINT: " https://trace/v1/traces " },
        "traces",
      ),
    ).toBe("https://trace/v1/traces");
    expect(
      resolveOtlpEndpoint(
        { OTEL_EXPORTER_OTLP_ENDPOINT: "https://base//" },
        "metrics",
      ),
    ).toBe("https://base/v1/metrics");
