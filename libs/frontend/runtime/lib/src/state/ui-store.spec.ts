...
type MatchMediaChangeListener = (event: MatchMediaChangeEvent) => void;

interface MatchMediaMockInstance {
  listeners: MatchMediaChangeListener[];
  matches: boolean;
  media: string;
  removeCalls: number;
...
function createMatchMediaMock(initialMatches: boolean, options: { legacyOnly?: boolean } = {}): MatchMediaMock {
  ...
  return {
    ...
    matchMedia: (query: string) => {
      ...
      const addListener = (listener: MatchMediaChangeListener) => {
        instance.listeners.push(listener);
      };
      const removeListener = (listener: MatchMediaChangeListener) => {
      ...
      if (!options.legacyOnly) {
        mediaQueryList['addEventListener'] = (_type: string, listener: MatchMediaChangeListener) => {
          addListener(listener);
        };
        mediaQueryList['removeEventListener'] = (_type: string, listener: MatchMediaChangeListener) => {
...
describe('UiStore', () => {
  ...
  it('removes the active system theme listener before changing subscriptions', () => {
    ...
    const store = new UiStore();
    const firstSubscription = matchMediaMock.instances.find((instance) => instance.listeners.length === 1);
    ...
    const activeSubscriptions = matchMediaMock.instances.filter((instance) => instance.listeners.length === 1);
  ...
  it('falls back to legacy media query listener APIs', () => {
    ...
    const subscription = matchMediaMock.instances.find((instance) => instance.listeners.length === 1);
