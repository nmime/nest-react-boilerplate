...
export function generateSummaryMd(summary: PlanSummary): { path: string; content: string } {
  ...
  lines.push(`- Slug: ${summary.identity.slug}`);
  lines.push(`- Domain: ${summary.identity.domain}`);
  lines.push(`- Owner: ${summary.identity.owner}`);
  lines.push(`- Brand slug: ${summary.identity.brand.s3Bucket}`);
...
export function generateComposeEnvironment(summary: PlanSummary): { path: string; content: string } {
  ...
  const dbName = summary.identity.dbName;
  const postgresPort = summary.runtime.ports.postgres ?? 5432;
  const mongoPort = summary.runtime.ports.mongodb ?? 27017;
  const lines = [
    ...
    ...(databaseProvider === 'mongodb'
      ? [
          ...
          `MONGODB_URI=mongodb://mongodb.localhost:${mongoPort}/${dbName}?replicaSet=rs0&retryWrites=true`,
    ...
    ...(databaseProvider === 'postgres'
      ? [
          `DATABASE_URL=postgres://postgres:postgres@localhost:${postgresPort}/${dbName}`,
          `CONTAINER_DATABASE_URL=postgres://postgres:postgres@postgres:${postgresPort}/${dbName}`,
        ]
      : []),
    ...
    `TENANCY_ENABLED=${summary.capabilities.includes('tenancy') ? 'true' : 'false'}`,
  ];
  return { path: '.nrb/capabilities.env', content: `${lines.join('\n')}\n` };
}

/**
 * Emits the registry of migrations owned by selected capabilities.
 *
 * Every other capability artifact is inert when the capability is deselected —
 * an unimported Nest module does nothing. Migrations are the exception: they
 * run against the customer's database regardless of whether the feature is
 * wired. Tenant row-level security is the case that proved it, shipping
 * `FORCE ROW LEVEL SECURITY` to single-tenant projects that had no runtime able
 * to satisfy the policies. Generating the list, rather than hardcoding it in
 * the migrator config, is what makes deselection mean "this DDL never runs".
 */
export function generateCapabilityMigrationRegistry(summary: PlanSummary): { path: string; content: string } {
  const provider = resolveDatabaseProvider(summary.capabilities);
  ...
  const imports = migrations.map((entry) => `import { ${entry.importName} } from '${entry.importPath}';`);
  const entries = migrations.map((entry) => `  { class: ${entry.importName}, name: '${entry.importName}' },`);
  ...
  return {
    path: 'packages/tooling/src/commands/db/capability-migrations.generated.ts',
    content: `${lines.join('\n')}\n`,
  };
}
...
export function plan(config: NrbConfig, currentState: SetupState = emptyState): PlanResult {
  ...
  const capabilityMigrationRegistry = generateCapabilityMigrationRegistry(summary);
  ...
  const desiredContent = new Map([
    ...
    [capabilityMigrationRegistry.path, capabilityMigrationRegistry.content],
