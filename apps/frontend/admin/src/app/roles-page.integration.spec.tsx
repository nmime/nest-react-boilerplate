...
describe("admin roles page management", () => {
  ...
  it("edits a role label and description", async () => {
    const updateSpy = vi
      .spyOn(adminApi, "adminRolesControllerUpdateRole")
      .mockResolvedValue(
        ok({
          id: "role-ops",
          role: "ops",
    ...
    expect(
      await screen.findByText(
        "Role update requested and the catalog will refetch.",
      ),
    ).toBeTruthy();
  });
  ...
  it("surfaces a backend rejection of a role update", async () => {
    vi.spyOn(adminApi, "adminRolesControllerUpdateRole").mockRejectedValue(
      new Error("role update backend rejected"),
    );
    ...
    expect(
      await screen.findByText("role update backend rejected"),
    ).toBeTruthy();
  });
  ...
  it("requires a role key before creating a role", async () => {
    ...
    expect(
      await screen.findByText("Enter a role key before creating a role."),
    ).toBeTruthy();
    expect(createSpy).not.toHaveBeenCalled();
  });

  it("surfaces a backend rejection of role creation", async () => {
    vi.spyOn(adminApi, "adminRolesControllerCreateRole").mockRejectedValue(
      new Error("role creation backend rejected"),
    );
