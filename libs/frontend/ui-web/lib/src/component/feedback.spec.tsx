...
const MaybeThrowingChild = ({ shouldThrow }: Readonly<{ shouldThrow: boolean }>) => {
  if (shouldThrow) {
...
describe('UiErrorBoundary', () => {
  ...
  it('supports custom fallbacks and reset-key recovery', async () => {
    ...
    const { rerender } = render(
      <UiErrorBoundary fallback={<div role="alert">Custom fallback</div>} onReset={onReset} resetKey="failed">
    ...
    rerender(
      <UiErrorBoundary fallback={<div role="alert">Custom fallback</div>} onReset={onReset} resetKey="recovered">
