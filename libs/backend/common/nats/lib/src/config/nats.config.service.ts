...
export class NatsConfigService {
  protected readonly configService = createConfig<NatsEnvironment>(natsEnvSchema);
  ...
  get maxReconnectAttempts(): number | undefined {
    return this.options.maxReconnectAttempts ?? this.configService.get('NATS_MAX_RECONNECT_ATTEMPTS');
  }

  get reconnectTimeWaitMs(): number | undefined {
    return this.options.reconnectTimeWaitMs ?? this.configService.get('NATS_RECONNECT_TIME_WAIT_MS');
  }

  get waitOnFirstConnect(): boolean | undefined {
    return this.options.waitOnFirstConnect ?? this.configService.get('NATS_WAIT_ON_FIRST_CONNECT');
  }

  get pingIntervalMs(): number | undefined {
    return this.options.pingIntervalMs ?? this.configService.get('NATS_PING_INTERVAL_MS');
  }

  get drainTimeoutMs(): number {
    return this.options.drainTimeoutMs ?? this.configService.get('NATS_DRAIN_TIMEOUT_MS');
  }
  ...
  private validateAuthentication(): void {
    if (this.token && (this.user || this.pass)) {
      throw new Error('NATS_TOKEN is mutually exclusive with NATS_USER/NATS_PASS.');
    }
