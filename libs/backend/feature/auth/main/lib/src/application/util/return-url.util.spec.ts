...
describe("return URL utilities", () => {
  ...
  it("normalizes hosts, validates ports and paths, and handles malformed allowlist entries", () => {
    ...
    expect(
      normalizeReturnUrlHost(new URL("https://APP.example.test./path")),
    ).toBe("app.example.test");
    expect(
      isReturnUrlAllowed("https://app.example.test:444/app/next", [
        "https://app.example.test:444/app",
      ]),
    ).toBe(true);
    expect(
      isReturnUrlAllowed("https://app.example.test:445/app/next", [
        "https://app.example.test:444/app",
      ]),
    ).toBe(false);
    expect(
      isReturnUrlAllowed("http://app.example.test/app/next", [
        "https://app.example.test/app",
      ]),
    ).toBe(false);
    expect(
      isReturnUrlAllowed("https://app.example.test/app", ["not a url"]),
    ).toBe(false);
