...
`@app/backend-common-redis` exports `SharedRateLimiter`, `SharedRateLimiterInjectToken`, and `buildRateLimitKey()` for tenant-aware limits. `RedisRateLimitService` implements the interface with `@redis/client`/in-memory clients.
