/* eslint-disable @typescript-eslint/unbound-method -- asserting on vitest mock fns exposed as typed client methods (expect(client.get)…), not invoking unbound methods */
...
describe("createRedisClient", () => {
  ...
  it("builds a single client from a url and identifies it by url with default db", () => {
    const client = createRedisClient(
      baseConfig({ url: "redis://redis-a:6379", db: undefined }),
    );
  ...
  it("requires at least one host for single mode without a url", () => {
    expect(() => createRedisClient(baseConfig({ hosts: [] }))).toThrow(
      "At least one Redis host is required.",
    );
  });
  ...
  it("builds a cluster client across all root nodes", () => {
    ...
    expect(createClusterMock).toHaveBeenCalledWith({
      rootNodes: [
        { socket: { host: "redis-a", port: 7000 } },
        { socket: { host: "redis-b", port: 7001 } },
      ],
  ...
  it("builds a sentinel client when a group identifier is present", () => {
    ...
    expect(createSentinelMock).toHaveBeenCalledWith(
      expect.objectContaining({ name: "mymaster", replicaPoolSize: 1 }),
    );
  });

  it("rejects sentinel mode without a group identifier", () => {
    expect(() =>
      createRedisClient(baseConfig({ mode: RedisMode.Sentinel })),
    ).toThrow("Redis sentinel mode requires a sentinel group identifier.");
  });
});
