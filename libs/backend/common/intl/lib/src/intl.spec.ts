...
import { randomIntlText, randomLocalizedText, resolveLocalizedText } from './random-intl-text.util';
...
describe('@app/backend-common-intl', () => {
  ...
  it('resolves bot locale headers for formatting/text selection context', () => {
    ...
    expect(resolver.resolve({ 'accept-language': 'ru-RU, en;q=0.7' })).toBe('ru-RU');
  ...
  it('selects localized text with fallback but no translation-key lookup', () => {
    ...
    expect(resolveLocalizedText({ ru: 'Привет', en: 'Hello' }, 'ru')).toBe('Привет');
  ...
  it('does not depend on i18n translation catalogs or lookup', () => {
    ...
    const textHelperSource = readFileSync(join(intlSourceRoot, 'random-intl-text.util.ts'), 'utf8');
