...
describe('S3Module', () => {
  it('provides a default in-memory client when none is supplied', async () => {
    ...
    expect(moduleRef.get(ObjectStorageInjectToken)).toBeInstanceOf(InMemoryObjectStorageClient);

    const service = moduleRef.get(S3Service);
    await service.putObject({ bucket: 'b', key: 'k', body: 'v' });
    await expect(service.getObject({ bucket: 'b', key: 'k' })).resolves.toMatchObject({ key: 'k' });
  });
