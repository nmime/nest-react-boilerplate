...
describe("auth role stores", () => {
  it("simulates role -> permission resolution in memory using the shared matrix", async () => {
    ...
    expect(
      sorted((await store.listRoleKeys("user-id"))._unsafeUnwrap()),
    ).toEqual([AdminRole, UserRole]);

    const access = (
      await store.resolveEffectiveAccess("user-id")
    )._unsafeUnwrap();
    expect(sorted(access.roleKeys)).toEqual([AdminRole, UserRole]);
    expect(access.permissionKeys).toEqual(
      permissionsForRoles([UserRole, AdminRole]),
    );
  });
  ...
  it("returns empty access for users without assignments and other tenants", async () => {
    ...
    expect(
      (
        await store.resolveEffectiveAccess("user-id", "other-tenant")
      )._unsafeUnwrap(),
    ).toEqual({ roleKeys: [], permissionKeys: [] });
  });
  ...
  it("delegates every call to the auth user role repository in Postgres mode", async () => {
    ...
    expect((await store.listRoleKeys("user-id"))._unsafeUnwrap()).toEqual([
      UserRole,
    ]);
    expect(repository.listRoleKeys).toHaveBeenCalledWith(
      "user-id",
      DefaultAuthTenantId,
    );

    expect(
      (await store.resolveEffectiveAccess("user-id"))._unsafeUnwrap(),
    ).toEqual({
      roleKeys: [UserRole],
      permissionKeys: [UserProfileReadPermission],
    });
    expect(repository.resolveEffectiveAccess).toHaveBeenCalledWith(
      "user-id",
      DefaultAuthTenantId,
    );
  });
