...
export class InMemoryAuthOAuthStateStore implements AuthOAuthStateStore {
  ...
  consumeState(input: {
    ...
    for (const state of states) {
      if (state.expiresAt <= input.now) continue;
    ...
    if (remaining.length > 0) this.statesBySession.set(input.sessionId, remaining);
    else this.statesBySession.delete(input.sessionId);
  ...
  private pruneExpired(now: number): void {
    for (const [sessionId, states] of this.statesBySession) {
      ...
      if (remaining.length > 0) this.statesBySession.set(sessionId, remaining);
      else this.statesBySession.delete(sessionId);
    }
  }
}
...
export class AuthOAuthService {
  ...
  buildAuthorizationRequest(
    ...
    if (!this.config.enabled) return errAsync({ code: "disabled", message: "OAuth is disabled." });

    const missing = this.getMissingConfiguration(["issuerUrl", "clientId", "redirectUri"]);
    if (missing.length > 0) {
      return errAsync({ code: "not_configured", message: `OAuth configuration is incomplete: ${missing.join(", ")}.` });
    }

    const sessionId = normalizeSessionId(input.sessionId);
    if (!sessionId) {
      return errAsync({ code: "invalid_request", message: "OAuth authorization requires a server-side session." });
    }

    const returnUrl = this.validateReturnUrl(input.returnUrl);
    if (isAuthOAuthError(returnUrl)) return errAsync(returnUrl);
    ...
    try {
      ...
      const authorizationUrl = new URL("authorize", this.config.issuerUrl as string);
      authorizationUrl.searchParams.set("client_id", this.config.clientId as string);
      authorizationUrl.searchParams.set("redirect_uri", this.config.redirectUri as string);
      ...
      return errAsync({ code: "provider_error", message: (error as Error).message });
    }
  }
  ...
  consumeAuthorizationState(
    ...
    if (isAuthOAuthError(consumed)) return errAsync(consumed);
  ...
  private consumeAuthorizationStateRecord(input: AuthOAuthCallbackInput): AuthOAuthStoredState | AuthOAuthError {
    ...
    if (!sessionId || !state) {
      return { code: "invalid_state", message: "OAuth callback state is required for this session." };
    }

    try {
      const consumed = this.stateStore.consumeState({ sessionId, stateHash: hashOAuthValue(state), now: this.now() });
      if (!consumed) {
        return { code: "invalid_state", message: "OAuth callback state is missing, expired, already used, or does not match this session." };
      }
      return consumed;
    } catch (error) {
  ...
  private validateReturnUrl(value: string | null | undefined): string | undefined | AuthOAuthError {
    if (value === null || value === undefined || value.trim() === "") return undefined;

    const normalized = normalizeReturnUrl(value);
    const allowed = (this.config.allowedReturnUrls ?? []).map(normalizeReturnUrl).filter((url): url is string => Boolean(url));

    if (!normalized || !allowed.includes(normalized)) {
      return { code: "invalid_request", message: "OAuth return URL is not allowlisted." };
    }
