...
const mockList = () =>
  vi
    ...
    .mockResolvedValue(ok({ items: [activeUser], total: 1, limit: 10, offset: 0 }));

const mockRoles = () => vi.spyOn(adminApi, 'adminUsersControllerRoles').mockResolvedValue(ok(rolesCatalog));
...
describe('admin users page interactions', () => {
  ...
  it('updates the list query from search input, submit, and filter selects', async () => {
    ...
    fireEvent.click(await screen.findByRole('option', { name: 'admin:users:read' }));
  ...
  it('requires a reason before requesting a status change and then submits it', async () => {
    mockList();
    mockRoles();
    vi.spyOn(adminApi, 'adminUsersControllerGetUser').mockResolvedValue(ok(activeUser));
    const statusSpy = vi
    ...
    expect(await screen.findByText('Enter a reason before changing status.')).toBeTruthy();
  ...
  it('surfaces a backend rejection of a status change', async () => {
    mockList();
    mockRoles();
    vi.spyOn(adminApi, 'adminUsersControllerUpdateUserStatus').mockRejectedValue(new Error('status backend rejected'));
  ...
  it('cancels the status dialog without mutating', async () => {
    mockList();
    mockRoles();
    const statusSpy = vi.spyOn(adminApi, 'adminUsersControllerUpdateUserStatus');
  ...
  it('edits an access policy by toggling roles and permissions', async () => {
    ...
    fireEvent.click(within(dialog).getByRole('checkbox', { name: 'admin:users:read' }));
  ...
  it('surfaces a backend rejection of an access policy update', async () => {
    mockList();
    mockRoles();
    vi.spyOn(adminApi, 'adminUsersControllerUpdateUserAccessPolicy').mockRejectedValue(
      new Error('policy backend rejected'),
    );
  ...
  it('validates reason and role selection before updating an access policy', async () => {
    mockList();
    mockRoles();
    const accessSpy = vi.spyOn(adminApi, 'adminUsersControllerUpdateUserAccessPolicy');
    ...
    openActionsMenu();
    fireEvent.click(await screen.findByRole('menuitem', { name: 'Edit access policy' }));
    fireEvent.click(await screen.findByRole('button', { name: 'Update access policy' }));
    expect(await screen.findByText('Enter a reason before changing access policy.')).toBeTruthy();
  ...
  it('renders users with empty access lists and a disabled status action', async () => {
    ...
    mockRoles();
    vi.spyOn(adminApi, 'adminUsersControllerGetUser').mockResolvedValue(ok(disabledUser));
    ...
    openActionsMenu();
    expect(await screen.findByRole('menuitem', { name: 'Change status' })).toBeTruthy();
  });

  it('uses access fallbacks when the roles catalog is unavailable', async () => {
    mockList();
    vi.spyOn(adminApi, 'adminUsersControllerRoles').mockRejectedValue(new Error('roles unavailable'));
    ...
    expect(within(policyDialog).getByRole('checkbox', { name: 'admin' })).toBeTruthy();
    expect(
      within(policyDialog).getByRole('checkbox', {
        name: 'admin:dashboard:read',
      }),
    ).toBeTruthy();
    fireEvent.click(within(policyDialog).getByRole('button', { name: 'Cancel' }));

    openActionsMenu();
    fireEvent.click(await screen.findByRole('menuitem', { name: 'Assign roles' }));
    const rolesDialog = await screen.findByRole('alertdialog');
    expect(within(rolesDialog).getByRole('checkbox', { name: 'admin' })).toBeTruthy();
  });

  it('renders the directory error state when the list request fails', async () => {
    vi.spyOn(adminApi, 'adminUsersControllerListUsers').mockRejectedValue(new Error('directory offline'));
    mockRoles();
