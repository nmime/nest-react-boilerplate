...
async function logComposeDiagnostics(label) {
  ...
  await run(
    "docker",
    [...compose, "logs", "--no-color", "--tail", "200", "migrate", "postgres"],
    { stdio: "inherit", env },
  ).catch(() => undefined);
}
...
async function composeUp() {
  ...
  const appServices = stackServices.filter(
    (service) => service !== "postgres" && service !== "migrate",
  );
  try {
    await run("docker", [...compose, "up", "--no-build", "-d", ...appServices], {
      stdio: "inherit",
      env,
    });
  } catch (error) {
    console.warn(
      `docker compose app startup reported a transient failure; retrying once: ${
        error instanceof Error ? error.message : String(error)
      }`,
    );
    await logComposeDiagnostics("docker compose app startup transient failure");
