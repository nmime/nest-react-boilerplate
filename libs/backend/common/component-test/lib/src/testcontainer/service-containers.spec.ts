...
const { DefaultRedisTestPort, createRedisContainer, startRedisContainer } = await import('./redis-container');
...
describe('service container helpers', () => {
  ...
  it('builds MySQL and Redis containers with defaults and start URLs', async () => {
    ...
    await expect(startRedisContainer({ image: 'redis:test', internalPort: 6380 })).resolves.toMatchObject({
      port: 16_380,
  ...
  it('builds NATS containers with optional JetStream and monitoring URLs', async () => {
    createNatsContainer();
    expect(lastContainer().command).toEqual(['-m', `${DefaultNatsMonitoringPort}`]);

    createNatsContainer({ jetStream: true, startupTimeoutMs: 15_000 });
    expect(lastContainer()).toMatchObject({
      command: ['-js', '-m', `${DefaultNatsMonitoringPort}`],
      exposedPorts: [DefaultNatsClientPort, DefaultNatsClientPort, DefaultNatsMonitoringPort],
