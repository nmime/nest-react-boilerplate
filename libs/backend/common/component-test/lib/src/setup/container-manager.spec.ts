...
describe('ContainerManager', () => {
    ...
    await expect(manager.stopAll()).rejects.toMatchObject({
      ...
      errors: [
        expect.objectContaining({
          message: 'Failed to stop managed test container at teardown index 1/3.',
          cause: secondFailure,
          container: third,
          stopIndex: 0,
          totalContainers: 3,
        ...
        expect.objectContaining({
          message: 'Failed to stop managed test container at teardown index 3/3.',
          cause: firstFailure,
          container: first,
          stopIndex: 2,
          totalContainers: 3,
  ...
  it('clears registered containers after a failed teardown attempt', async () => {
    ...
    await expect(manager.stopAll()).rejects.toThrow('Failed to stop 1 of 1 managed test containers.');
    await manager.stopAll();
