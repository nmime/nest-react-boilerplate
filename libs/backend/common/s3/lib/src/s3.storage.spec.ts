...
describe("InMemoryObjectStorageClient", () => {
  it("stores a string body as bytes and reads it back", async () => {
    ...
    expect(Buffer.from(object?.body ?? new Uint8Array()).toString()).toBe(
      "hello",
    );
  ...
  it("returns null for a missing object", async () => {
    ...
    await expect(
      client.getObject({ bucket: "media", key: "missing" }),
    ).resolves.toBeNull();
  });
  ...
  it("deletes an object", async () => {
    ...
    await expect(
      client.getObject({ bucket: "media", key: "temp" }),
    ).resolves.toBeNull();
  });
  ...
  it("isolates objects by bucket when listing", async () => {
    ...
    expect(
      objects.map((object) => object.key).sort((a, b) => a.localeCompare(b)),
    ).toEqual(["a", "b"]);
  });
