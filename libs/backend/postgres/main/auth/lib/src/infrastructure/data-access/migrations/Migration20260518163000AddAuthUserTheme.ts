...
export class Migration20260518163000AddAuthUserTheme extends Migration {
  override up(): void {
    this.addSql('alter table "auth_users" add column if not exists "theme" varchar(16) not null default \'system\';');
    this.addSql(`
      do $$
      begin
        if not exists (
  ...
  override down(): void {
    this.addSql('alter table "auth_users" drop constraint if exists "ck__auth_users__theme";');
