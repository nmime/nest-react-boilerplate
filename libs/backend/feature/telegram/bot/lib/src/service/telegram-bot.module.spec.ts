...
function hasConfigToken(
  value: unknown,
): value is { config: { token: string } } {
  return (
    typeof value === "object" &&
    value !== null &&
    "config" in value &&
...
describe("TelegramBotModule", () => {
  ...
  it("declares and exports the bot instance provider", () => {
    ...
    const provider = providers.find(
      (candidate) => candidate.provide === TelegramBotInstanceInjectToken,
    );
    expect(typeof provider?.useFactory).toBe("function");

    const exportsMetadata: unknown = Reflect.getMetadata(
