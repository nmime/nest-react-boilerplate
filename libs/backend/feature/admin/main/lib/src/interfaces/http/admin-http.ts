...
import { createAdminRequestContext, type AdminRequestContext } from '../../domain';
...
export const toHttpException = (error: unknown): never => {
  if (error instanceof AdminApplicationError) {
    ...
    if (error.code === 'invalid_access_policy' || error.code === 'sensitive_policy_violation') {
...
export const executeAdminUseCase = async <T>(handler: () => Promise<T>): Promise<T> => {
  try {
