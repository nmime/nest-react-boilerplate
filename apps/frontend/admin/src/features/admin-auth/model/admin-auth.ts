...
import { getAdminApiBaseUrl, getAuthApiBaseUrl } from '../../../entities/admin-session';
...
export const stripSensitiveBrowserTokenParams = (): void => {
  ...
  const hadSensitiveParams = sensitiveUrlTokenParams.some((param) => url.searchParams.has(param));
...
export const getConfiguredAuthApiBaseUrl = (): string => getAuthApiBaseUrl(getFrontendEnv());
