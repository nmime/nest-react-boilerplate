...
import { UiButton, UiCard, UiSection, type ProductShellAction } from '@app/frontend-ui-web';
import { LandingStatCard } from '../../../shared/ui';

const overviewHeading = 'A launch cockpit for teams turning the boilerplate into a production product';
const valueGridLabel = 'Landing v3 product value pillars';
const featureGridLabel = 'Platform architecture and release workflow highlights';
...
const routePanelHeading = 'Explore every shipped surface without losing landing context';
...
const readinessHeading = 'Designed to feel product-ready before the first sprint begins';
...
const workflowSteps = [
  {
    description: 'Review the landing narrative, app shell links, and environment fallbacks before adding domain copy.',
    kicker: '01 / Discover',
...
const operationsItems = [
  ...
  {
    detail: "Primary and admin application routes remain the landing page's headline CTAs and smoke-test anchors.",
    label: '/app + /admin',
...
const actionTones = ['Primary launch path', 'Admin control plane', 'API reference'] as const;
...
export const ProductOverview = ({ actions }: Readonly<ProductOverviewProps>) => {
  const { t } = useI18n();
    ...
    <div className="landing-surface" id="workspace">
      ...
      >
        <div className="landing-overview__intro">
          <p>
            Landing v3 frames the repository as a product launch system: clear narrative, route readiness, validation
            confidence, and polished responsive presentation in one scannable surface.
          </p>
          <div className="landing-overview__badges" aria-label="Landing v3 markers">
            <span>Design v3</span>
        ...
        <div className="landing-value-grid" aria-label={valueGridLabel}>
          {valueCards.map((card) => (
            <UiCard className="landing-value-card" key={card.heading} title={card.heading}>
        ...
        <div aria-label={t('landing.statGrid.label')} className="landing-stat-grid">
      ...
      >
        {featureSections.map((section) => (
          <UiCard className="landing-feature-card" key={section.heading}>
            <p className="landing-feature-card__eyebrow">{section.eyebrowText}</p>
      ...
      <section aria-label={journeyLabel} className="landing-journey" data-smoke-marker="landing-v3-journey">
