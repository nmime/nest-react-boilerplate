...
const createController = () => {
  const roles = {
    listRolesWithPermissions: vi.fn(() =>
      okAsync([
        { role: adminRole, permissionKeys: [AdminRolesWritePermission] },
      ]),
    ),
    ...
    findByKeys: vi.fn(() =>
      okAsync([{ ...adminRole, key: UserRole }, adminRole]),
    ),
    createRole: vi.fn(() =>
      okAsync({
        ...adminRole,
  ...
  return {
    roles,
    adminUserMutations,
    controller: new AdminRolesController(
      new AdminRolesUseCase(roles as never, adminUserMutations as never),
    ),
  };
};
...
describe("AdminRolesController", () => {
  ...
  it("creates a role", async () => {
    ...
    await expect(
      controller.createRole(principal, { key: "support" }),
    ).resolves.toMatchObject({ data: { role: "support" } });
  });
  ...
  it("sets role permissions when invariants are preserved", async () => {
    ...
    const response = await controller.setRolePermissions(
      principal,
      "role-admin",
      {
        permissions: [
  ...
  it("updates a role's label", async () => {
    ...
    await expect(
      controller.updateRole(principal, "role-admin", { label: "Ops" }),
    ).resolves.toMatchObject({ data: { role: AdminRole } });
  });
