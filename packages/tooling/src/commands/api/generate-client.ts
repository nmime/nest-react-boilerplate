...
function parseArgs(argv) { const args = { input: OPENAPI_CONTRACT_FILES.auth, output: "libs/frontend/api-client/lib/src/generated.ts", dryRun: false }; for (let i = 0; i < argv.length; i += 1) { const item = argv[i]; if (item === "--") continue; const val = () => { const next = argv[++i]; if (!next) throw new Error(`${item} requires a value.`); return next; }; if (item === "--input") args.input = val(); else if (item === "--output") args.output = val(); else if (item === "--dry-run") args.dryRun = true; else if (item === "--help" || item === "-h") args.help = true; else throw new Error(`Unknown option: ${item}`); } return args; }
const args = parseArgs(process.argv.slice(2));
if (args.help) { console.log("Usage: pnpm api:client -- [--input libs/common/api-contracts/openapi/auth-app-api.json] [--output libs/frontend/api-client/lib/src/generated.ts]"); process.exit(0); }
