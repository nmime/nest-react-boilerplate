#!/usr/bin/env node
import { commandExists, ensureDir, envList, parseArgs, run, writeJson } from "./runtime-utils.mjs";
...
if (!urls.length && !apiUrls.length) {
  writeJson(reportPath, { status: "failed", engine, urls, apiUrls, budget, error: "No performance targets configured." });
  console.error("No performance targets configured. Set PERF_URLS and/or PERF_API_URLS to real HTTP(S) targets.");
  process.exit(1);
}
