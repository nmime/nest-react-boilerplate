...
describe('frontend platform utilities', () => {
  ...
  it('returns false when no navigator is available', async () => {
  ...
  it('formats pay time tokens', () => {
    ...
    expect(formatPayTime('day_2')).toBe('2 days');
    expect(formatPayTime('day_1')).toBe('1 day');
  ...
  it('creates exact-first search functions across selected fields', () => {
    ...
    expect(createSearchFunction<(typeof items)[number]>([])(items, 'usd')).toEqual(items);
  });

  it('skips nil fields when matching search terms', () => {
    const search = createSearchFunction<{
      code: string | null;
      name?: string;
    }>(['code', 'name']);
    const items = [
      { code: null, name: undefined },
      { code: 'USD', name: 'Dollar' },
      { code: null, name: 'usd stablecoin' },
    ];
  ...
  it('opens existing URLs only when a browser opener is available', () => {
    ...
    const open = vi.spyOn(window, 'open').mockImplementation(() => openedWindow);
