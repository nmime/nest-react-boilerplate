...
describe("FeatureFlagEntity", () => {
  ...
  it("defines timestamp lifecycle hooks", () => {
    ...
    expect(
      FeatureFlagEntitySchema.meta.properties.createdAt.onCreate?.(),
    ).toBeInstanceOf(Date);
    expect(
      FeatureFlagEntitySchema.meta.properties.updatedAt.onCreate?.(),
    ).toBeInstanceOf(Date);
    expect(
      FeatureFlagEntitySchema.meta.properties.updatedAt.onUpdate?.(),
    ).toBeInstanceOf(Date);
  });
});
