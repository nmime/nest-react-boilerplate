...
function generateTypes({ input, output }: { input: string; output: string }) { mkdirSync(dirname(output), { recursive: true }); const result = spawnSync("pnpm", ["exec", "openapi-typescript", input, "-o", output], { stdio: "inherit" }); if (result.status !== 0) process.exit(result.status ?? 1); }
function format(paths: string[]) { const result = spawnSync("pnpm", ["exec", "prettier", "--write", "--ignore-path", "/dev/null", ...paths.map((path) => resolve(path))], { stdio: "inherit" }); if (result.status !== 0) process.exit(result.status ?? 1); }
function artifactOutput(contract: OpenApiContract, args: GenerateContractsArgs) { return args.contractsRoot ? join(args.contractsRoot, `${contract.name}.json`) : contract.artifactPath; }
function typesOutput(contract: OpenApiContract, args: GenerateContractsArgs) { return args.typesRoot ? join(args.typesRoot, `${contract.name}.ts`) : contract.typesPath; }
