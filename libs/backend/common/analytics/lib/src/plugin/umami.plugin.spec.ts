...
describe('createUmamiAnalyticsPlugin', () => {
  ...
  it('throws when Umami rejects the event', async () => {
    ...
    await expect(plugin.track?.({ event: 'failed' })).rejects.toThrow('Umami analytics request failed: 502');
  });

  it('rejects missing endpoint and host instead of constructing a relative URL', () => {
    expect(() => createUmamiAnalyticsPlugin({ websiteId: 'website-id' })).toThrow(
      'Umami analytics requires either endpoint or host',
    );
  });
});
