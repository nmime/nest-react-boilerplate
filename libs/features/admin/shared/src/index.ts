...
export type AdminPermission =
  ...
  | typeof ADMIN_LEGACY_READ_PERMISSION;
...
export const createAdminAccessPolicy = (
  ...
  const hasLegacyRead = permissions.includes(ADMIN_LEGACY_READ_PERMISSION);
  const canReadProfile =
    hasAdminRole &&
    (hasLegacyRead || permissions.includes(ADMIN_PROFILE_READ_PERMISSION));
  const canReadDashboard =
    hasAdminRole &&
    (hasLegacyRead || permissions.includes(ADMIN_DASHBOARD_READ_PERMISSION));
...
export const toAdminProfileView = (
  ...
  return {
    ...
    roles: createAdminAccessPolicy(principal).roles,
    permissions: createAdminAccessPolicy(principal).permissions,
  };
};
