...
describe('auth password hash verification', () => {
  ...
  it('rejects malformed or unsafe stored hashes without throwing', () => {
    ...
    expect(verifyPassword('password123', 'pbkdf2_sha256$NaN$salt$digest')).toBe(false);
    expect(verifyPassword('password123', 'pbkdf2_sha256$0$salt$digest')).toBe(false);
    expect(verifyPassword('password123', 'pbkdf2_sha256$1000001$salt$digest')).toBe(false);
    expect(verifyPassword('password123', 'pbkdf2_sha256$120000$salt$')).toBe(false);
    expect(verifyPassword('password123', `pbkdf2_sha256$120000$salt$${oversizedDigest}`)).toBe(false);
  });
});
