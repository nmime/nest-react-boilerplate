...
describe('AnalyticsConfigService', () => {
  ...
  it('creates GA4, PostHog, and Umami plugins when credentials are present', () => {
    ...
    expect(service.plugins.map((plugin) => plugin.name)).toEqual(['ga4-measurement-protocol', 'posthog', 'umami']);
  });
  ...
  it('reports the production flag from the resolved environment', () => {
    expect(new AnalyticsConfigService({ environment: 'production' }).isProduction).toBe(true);
    expect(new AnalyticsConfigService({ environment: 'staging' }).isProduction).toBe(false);
  });
  ...
  it('throws on malformed boolean and provider environment values', () => {
    ...
    expect(() => new AnalyticsConfigService().enabled).toThrow(/Invalid environment configuration.*ANALYTICS_ENABLED/u);
