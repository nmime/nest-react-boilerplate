...
describe('withLandingProviders', () => {
  it('uses an explicit component displayName', () => {
    ...
    expect(Wrapped.displayName).toBe('withLandingProviders(ExplicitLandingChild)');
    expect(renderToStaticMarkup(<Wrapped />)).toContain('named landing provider child');
  });

  it('falls back to Component when the wrapped component has no name', () => {
    const Anonymous = (() => <span>anonymous landing provider child</span>) as ComponentType;
    ...
    expect(renderToStaticMarkup(<Wrapped />)).toContain('anonymous landing provider child');
  });
});
