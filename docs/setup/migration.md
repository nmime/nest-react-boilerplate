...
Migrate from legacy `init:project` and `generate:feature` scripts to the NRB setup engine (`nrb setup` / `nrb add`).
...
- **`init:project` still works**: the legacy command remains in `packages/tooling` and continues to replace boilerplate tokens. It does not conflict with `nrb setup`.
- **`generate:feature` still works**: the legacy `pnpm generate:feature` alias routes through `project:generate-vertical-slice`, which is the same engine `nrb add feature` uses.
- **State is isolated**: `nrb setup` tracks state in `.nrb/state.json`. The legacy `init:project` uses Git to track changes. They do not interfere.
- **Idempotency**: `nrb setup` is safe to re-run. The planner produces zero operations if the workspace is already up to date.
...
The generated files are identical — both paths use the same `generate-vertical-slice` engine.

### Additional options

The `nrb add feature` command adds `--force` and `--api-app` flags:
...
- [ ] Run `nrb doctor` to verify workspace health.
- [ ] If you previously ran `init:project`, skip manual token replacement for files already processed.
- [ ] Run `nrb setup --dry-run` to see what the engine would do.
- [ ] Run `nrb setup` (interactive) or with `--config` (non-interactive).
- [ ] Verify with `nrb doctor --json` that `nrb-config` and `nrb-state` checks now pass.
- [ ] Use `nrb add feature <name> --dry-run` for future feature scaffolding.
