...
const hasConfiguredApiOrigin = (key: string): boolean => readEnvString(key).replace(/\/$/u, '').length > 0;
...
export const getUserAppApiModeLabel = (): string => {
  ...
  if (hasConfiguredApiOrigin('VITE_AUTH_API_BASE_URL') && hasConfiguredApiOrigin('VITE_USER_API_BASE_URL')) {
    return 'Explicit API origins';
  }

  const mode = readEnvString('MODE').toLowerCase();
  if (getFrontendEnv()['DEV'] === true || mode === 'development' || mode === 'test') {
