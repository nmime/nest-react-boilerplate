...
import { initializePaymentsCollection, verifyPaymentsCollection } from '../payments-mongo.collection';
...
export const Migration20260823100000InitializePayments: MongoMigration = {
  ...
  async up(database: Db): Promise<void> {
    await initializePaymentsCollection(database);
  },

  async verify(database: Db): Promise<void> {
    await verifyPaymentsCollection(database);
  },
};
