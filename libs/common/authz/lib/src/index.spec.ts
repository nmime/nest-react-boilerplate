...
describe('@app/common-authz normalizeStringList', () => {
  ...
  it('trims, drops empties/non-strings, and de-duplicates array input', () => {
    expect(normalizeStringList([' admin ', '', 'admin', null, 7, 'user'])).toEqual(['admin', 'user']);
  });
});
...
describe('@app/common-authz role matrix', () => {
  ...
  it('grants the user role read-only profile access', () => {
    expect(defaultRolePermissions[UserRole]).toEqual([UserProfileReadPermission]);
  });

  it('grants the admin role the full admin catalog including manage-all and roles:write', () => {
    expect(defaultRolePermissions[AdminRole]).toContain(AdminManageAllPermission);
    expect(defaultRolePermissions[AdminRole]).toContain(AdminRolesWritePermission);
    expect(defaultRolePermissions[AdminRole]).not.toContain(UserProfileReadPermission);
  });
