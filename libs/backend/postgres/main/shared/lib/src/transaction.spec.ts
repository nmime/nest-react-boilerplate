...
describe('runInPostgresTransaction', () => {
  it('returns the handler value when the transaction commits', async () => {
    ...
    const result = await runInPostgresTransaction(transactionalManager, (current) =>
      Promise.resolve({
  ...
  it('maps thrown errors to explicit transaction errors', async () => {
    ...
    const result = await runInPostgresTransaction(transactionalManager, () => Promise.resolve('unused'));
