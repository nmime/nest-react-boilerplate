...
6. Applications and libraries do not add package identity manifests. A
   dependency-only renderer manifest is allowed only when the renderer requires
   nearest-package metadata. If the scaffold needs a missing external package,
   add it to the owning backend/frontend platform or renderer boundary, run
   `pnpm install`, then prove
   `pnpm install --frozen-lockfile`; never hand-edit `pnpm-lock.yaml`.
