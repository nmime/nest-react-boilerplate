...
import { createTelegramApplication, resolveTelegramApplication } from './application';
import type { TelegramBotAuthPort, TelegramBotIdentity } from '../type';
...
describe('Telegram application adapter', () => {
  it('adapts transport requests to the configured auth boundary', async () => {
    ...
    const findLinkedUser = vi.fn().mockResolvedValue({ userId: 'user-1', tenantId: 'tenant-1' });
    ...
    await expect(application.consumeStartPayload('payload', identity)).resolves.toEqual({
      kind: 'link',
    });
    await expect(application.createLinkInstructions(identity)).resolves.toBe('open the app');
  ...
  it('keeps handlers safe when an application integration is not installed', async () => {
    ...
    await expect(application.consumeStartPayload('payload', identity)).resolves.toBeNull();
    await expect(application.createLinkInstructions(identity)).resolves.toBeNull();
    await expect(application.findLinkedUser(identity)).resolves.toBeNull();
    await expect(application.updateLinkedUserLocale({ identity, locale: 'en' })).resolves.toBeUndefined();
  });
  ...
  it('creates an adapter when dependencies are passed to the resolver', async () => {
    const createLinkInstructions = vi.fn().mockResolvedValue('link from deps');
    const application = resolveTelegramApplication({
      auth: {
        consumeLinkPayload: vi.fn().mockResolvedValue(null),
        createLinkInstructions,
        findLinkedUser: vi.fn().mockResolvedValue(null),
        updateLinkedUserLocale: vi.fn().mockResolvedValue(undefined),
      },
    });

    await expect(application.createLinkInstructions(identity)).resolves.toBe('link from deps');
    expect(createLinkInstructions).toHaveBeenCalledWith(identity);
  });
});
