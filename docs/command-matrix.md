...
| Deployment config validation | `node scripts/validate-deployment-config.mjs` | Docker, Helm, env, or deployment changes  | Runs static assertions for Compose, Helm, env examples, nginx, and runtime hardening; CI runs it before Helm rendering. |
